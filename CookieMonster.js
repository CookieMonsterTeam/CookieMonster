...
CM.Cache.CacheWrinklers = () => {
    ...
    for (let i; i < Game.wrinklers.length; i++) {
        ...
        if (CM.Sim.Objects.Temple.minigameLoaded) {
            const godLvl = CM.Sim.hasGod('scorn');
            if (godLvl === 1) {
                sucked *= 1.15;
            } else if (godLvl === 2) {
                sucked *= 1.1;
        ...
        if (Game.wrinklers[i].type === 0) {
            ...
            if (sucked > CM.Cache.WrinklersFattest[0]) {
                CM.Cache.WrinklersFattest = [sucked, i];
            }
        }
    }
};
 ...
 */
CM.Cache.CacheStats = () => {
    const goldenMult = CM.Cache.GoldenCookiesMult;
    const wrathMult = CM.Cache.WrathCookiesMult;

    CM.Cache.Lucky = (CM.Cache.NoGoldSwitchCookiesPS * 900) / 0.15;
    CM.Cache.Lucky *= CM.Cache.DragonsFortuneMultAdjustment;
    const cpsBuffMult = CM.Sim.getCPSBuffMult();
    if (cpsBuffMult > 0) {
    ...
    CM.Cache.LuckyRewardFrenzy =
        goldenMult * (CM.Cache.LuckyFrenzy * 0.15) + 13;
    CM.Cache.LuckyWrathRewardFrenzy =
        wrathMult * (CM.Cache.LuckyFrenzy * 0.15) + 13;
    ...
    for (const i of Object.keys(Game.Objects)) {
        if (Game.Objects[i].amount > max) {
            max = Game.Objects[i].amount;
        }
...
CM.Cache.CacheMissingUpgrades = () => {
    ...
    for (const i of Object.keys(list)) {
        ...
        if (me.bought === 0) {
            ...
            if (me.pool === 'prestige') {
                CM.Cache.MissingUpgradesPrestige += str;
            } else if (me.pool === 'cookie') {
                CM.Cache.MissingUpgradesCookies += str;
            } else if (
                me.pool !== 'toggle' &&
                me.pool !== 'unused' &&
                me.pool !== 'debug'
            ) {
                CM.Cache.MissingUpgrades += str;
            }
        }
    }
};
...
class CMAvgQueue {
    ...
    calcAverage(timePeriod) {
        if (timePeriod > this.maxLength) {
            timePeriod = this.maxLength;
        }
        if (timePeriod > this.queue.length) {
            timePeriod = this.queue.length;
        }
        let ret = 0;
        for (
...
CM.Cache.InitCookiesDiff = () => {
    CM.Cache.CookiesDiff = new CMAvgQueue(
        CM.Disp.cookieTimes[CM.Disp.cookieTimes.length - 1],
    );
    CM.Cache.WrinkDiff = new CMAvgQueue(
        CM.Disp.cookieTimes[CM.Disp.cookieTimes.length - 1],
    );
    CM.Cache.WrinkFattestDiff = new CMAvgQueue(
        CM.Disp.cookieTimes[CM.Disp.cookieTimes.length - 1],
    );
    CM.Cache.ChoEggDiff = new CMAvgQueue(
        CM.Disp.cookieTimes[CM.Disp.cookieTimes.length - 1],
    );
    CM.Cache.ClicksDiff = new CMAvgQueue(
        CM.Disp.clickTimes[CM.Disp.clickTimes.length - 1],
    );
};
...
CM.Cache.UpdateAvgCPS = () => {
    ...
    if ((Game.T / Game.fps) % 1 === 0) {
        ...
        if (CM.Cache.lastDate !== -1) {
            const timeDiff = currDate - CM.Cache.lastDate;
            const bankDiffAvg =
                Math.max(0, Game.cookies - CM.Cache.lastCookies) / timeDiff;
            const wrinkDiffAvg =
                Math.max(
                    0,
                    CM.Cache.WrinklersTotal - CM.Cache.lastWrinkCookies,
                ) / timeDiff;
            const wrinkFattestDiffAvg =
                Math.max(
                    0,
                    CM.Cache.WrinklersFattest[0] -
                        CM.Cache.lastWrinkFattestCookies,
                ) / timeDiff;
            const choEggDiffAvg =
                Math.max(0, choEggTotal - CM.Cache.lastChoEgg) / timeDiff;
            const clicksDiffAvg =
                (Game.cookieClicks - CM.Cache.lastClicks) / timeDiff;
        ...
        CM.Cache.AverageGainWrinkFattest = CM.Cache.WrinkFattestDiff.calcAverage(
            cpsLength,
        );
        ...
        if (CM.Options.CalcWrink === 1) {
            CM.Cache.AvgCPS += CM.Cache.AverageGainWrink;
        }
        if (CM.Options.CalcWrink === 2) {
            CM.Cache.AvgCPS += CM.Cache.AverageGainWrinkFattest;
        }
        ...
        if (choEgg || CM.Options.CalcWrink === 0) {
            CM.Cache.AvgCPSWithChoEgg =
                CM.Cache.AverageGainBank +
                CM.Cache.AverageGainWrink +
                (choEgg ? CM.Cache.AverageGainChoEgg : 0);
        } else {
            CM.Cache.AvgCPSWithChoEgg = CM.Cache.AvgCPS;
        }

        CM.Cache.AverageClicks = CM.Cache.ClicksDiff.calcAverage(
            CM.Disp.clickTimes[CM.Options.AvgClicksHist],
        );
    }
};
...
/**
 ...
 It is called by the relevant tooltip-code as a result of CM.Disp.AddDragonLevelUpTooltip() and by CM.Loop()
            ...
            if (target !== 'i') {
                ...
                if (Game.Objects[target].amount < amount) {
                    ...
                    for (let i = 0; i < amount; i++) {
                        let price =
                            CM.Sim.Objects[target].basePrice *
                            Game.priceIncrease **
                                Math.max(
                                    0,
                                    CM.Sim.Objects[target].amount -
                                        1 -
                                        CM.Sim.Objects[target].free,
                                );
                        price = Game.modifyBuildingPrice(
                            CM.Sim.Objects[target],
                            price,
                        );
                        price = Math.ceil(price);
                        cost += price;
                        CM.Sim.Objects[target].amount -= 1;
                    }
                    CM.Cache.CostDragonUpgrade = `Cost to rebuy: ${CM.Disp.Beautify(
                        cost,
                    )}`;
                }
            } else {
...
CM.Cache.NextNumber = base => {
    let count =
        base > 2 ** 53 ? 2 ** Math.floor(Math.log(base) / Math.log(2)) - 53 : 1;
...
CM.Cache.RemakeBuildingsPrices = () => {
    for (const i of Object.keys(Game.Objects)) {
        CM.Cache.Objects[i].price = CM.Sim.BuildingGetPrice(
            Game.Objects[i],
        ...
        CM.Cache.Objects10[i].price = CM.Sim.BuildingGetPrice(
            Game.Objects[i],
            Game.Objects[i].basePrice,
            Game.Objects[i].amount,
            Game.Objects[i].free,
        ...
        CM.Cache.Objects100[i].price = CM.Sim.BuildingGetPrice(
            Game.Objects[i],
            Game.Objects[i].basePrice,
            Game.Objects[i].amount,
            Game.Objects[i].free,
...
CM.Cache.RemakeBuildingsPP = () => {
    ...
    if (CM.Options.ColorPPBulkMode === 0) {
        for (const i of Object.keys(CM.Cache.Objects)) {
            // CM.Cache.Objects[i].pp = Game.Objects[i].getPrice() / CM.Cache.Objects[i].bonus;
            if (Game.cookiesPs) {
                CM.Cache.Objects[i].pp =
                    Math.max(
                        Game.Objects[i].getPrice() -
                            (Game.cookies + CM.Disp.GetWrinkConfigBank()),
                        0,
                    ) /
                        Game.cookiesPs +
                    Game.Objects[i].getPrice() / CM.Cache.Objects[i].bonus;
            } else {
                CM.Cache.Objects[i].pp =
                    Game.Objects[i].getPrice() / CM.Cache.Objects[i].bonus;
            }
            if (CM.Cache.min === -1 || CM.Cache.Objects[i].pp < CM.Cache.min) {
                CM.Cache.min = CM.Cache.Objects[i].pp;
            }
            if (CM.Cache.max === -1 || CM.Cache.Objects[i].pp > CM.Cache.max) {
                CM.Cache.max = CM.Cache.Objects[i].pp;
            }
        }
        ...
        for (const i of Object.keys(CM.Cache.Objects)) {
            ...
            } else if (CM.Cache.Objects[i].pp === CM.Cache.max) {
                color = CM.Disp.colorRed;
            } else if (CM.Cache.Objects[i].pp > CM.Cache.mid) {
    ...
    // Calculate PP and colors when compared to purchase of selected bulk mode
        ...
        for (const i of Object.keys(CM.Cache.Objects)) {
            if (Game.cookiesPs) {
                CM.Cache.Objects10[i].pp =
                    Math.max(
                        Game.Objects[i].bulkPrice -
                            (Game.cookies + CM.Disp.GetWrinkConfigBank()),
                        0,
                    ) /
                        Game.cookiesPs +
                    Game.Objects[i].bulkPrice / CM.Cache.Objects10[i].bonus;
            } else {
                CM.Cache.Objects10[i].pp =
                    Game.Objects[i].bulkPrice / CM.Cache.Objects10[i].bonus;
            }
            if (
                CM.Cache.min === -1 ||
                CM.Cache.Objects10[i].pp < CM.Cache.min
            ) {
                CM.Cache.min = CM.Cache.Objects10[i].pp;
            }
            if (
                CM.Cache.max === -1 ||
                CM.Cache.Objects10[i].pp > CM.Cache.max
            ) {
                CM.Cache.max = CM.Cache.Objects10[i].pp;
            }
        }
        CM.Cache.mid = (CM.Cache.max - CM.Cache.min) / 2 + CM.Cache.min;
        for (const i of Object.keys(CM.Cache.Objects)) {
            let color = '';
            if (CM.Cache.Objects10[i].pp === CM.Cache.min) {
                color = CM.Disp.colorGreen;
            } else if (CM.Cache.Objects10[i].pp === CM.Cache.max) {
                color = CM.Disp.colorRed;
            } else if (CM.Cache.Objects10[i].pp > CM.Cache.mid) {
                color = CM.Disp.colorOrange;
            } else {
        ...
        for (const i of Object.keys(CM.Cache.Objects)) {
            if (Game.cookiesPs) {
                CM.Cache.Objects100[i].pp =
                    Math.max(
                        ...
                        Game.cookiesPs +
                    Game.Objects[i].bulkPrice / CM.Cache.Objects100[i].bonus;
            } else {
                CM.Cache.Objects100[i].pp =
                    Game.Objects[i].bulkPrice / CM.Cache.Objects100[i].bonus;
            }
            if (
                CM.Cache.min === -1 ||
                CM.Cache.Objects100[i].pp < CM.Cache.min
            ) {
                CM.Cache.min = CM.Cache.Objects100[i].pp;
            }
            if (
                CM.Cache.max === -1 ||
                CM.Cache.Objects100[i].pp > CM.Cache.max
            ) {
                CM.Cache.max = CM.Cache.Objects100[i].pp;
            }
        }
        CM.Cache.mid = (CM.Cache.max - CM.Cache.min) / 2 + CM.Cache.min;
        for (const i of Object.keys(CM.Cache.Objects)) {
            let color = '';
            if (CM.Cache.Objects100[i].pp === CM.Cache.min) {
                color = CM.Disp.colorGreen;
            } else if (CM.Cache.Objects100[i].pp === CM.Cache.max) {
                color = CM.Disp.colorRed;
            } else if (CM.Cache.Objects100[i].pp > CM.Cache.mid) {
                color = CM.Disp.colorOrange;
            } else {
                color = CM.Disp.colorYellow;
            }
        ...
        if (Game.cookiesPs) {
            CM.Cache.Upgrades[i].pp =
                Math.max(
                    ...
                    Game.cookiesPs +
                Game.Upgrades[i].getPrice() / CM.Cache.Upgrades[i].bonus;
        } else {
            CM.Cache.Upgrades[i].pp =
                Game.Upgrades[i].getPrice() / CM.Cache.Upgrades[i].bonus;
        }
        if (Number.isNaN(CM.Cache.Upgrades[i].pp)) {
            CM.Cache.Upgrades[i].pp = Infinity;
        }
        let color = '';
        if (
            CM.Cache.Upgrades[i].pp <= 0 ||
            CM.Cache.Upgrades[i].pp === Infinity
        ) {
            ...
            color = CM.Disp.colorGreen;
        } else if (CM.Cache.Upgrades[i].pp === CM.Cache.max) {
            color = CM.Disp.colorRed;
        } else if (CM.Cache.Upgrades[i].pp > CM.Cache.max) {
            color = CM.Disp.colorPurple;
        } else if (CM.Cache.Upgrades[i].pp > CM.Cache.mid) {
            color = CM.Disp.colorOrange;
        } else {
            color = CM.Disp.colorYellow;
        }
...
CM.Cache.RemakeBuildingsOtherPP = (amount, target) => {
    for (const i of Object.keys(CM.Cache[target])) {
        // CM.Cache[target][i].pp = CM.Cache[target][i].price / CM.Cache[target][i].bonus;
        if (Game.cookiesPs) {
            CM.Cache[target][i].pp =
                Math.max(
                    CM.Cache[target][i].price -
                        (Game.cookies + CM.Disp.GetWrinkConfigBank()),
                    0,
                ) /
                    Game.cookiesPs +
                CM.Cache[target][i].price / CM.Cache[target][i].bonus;
        } else {
            CM.Cache[target][i].pp =
                CM.Cache[target][i].price / CM.Cache[target][i].bonus;
        }
        let color = '';
        if (
            CM.Cache[target][i].pp <= 0 ||
            CM.Cache[target][i].pp === Infinity
        ) {
            color = CM.Disp.colorGray;
        } else if (CM.Cache[target][i].pp < CM.Cache.min) {
            color = CM.Disp.colorBlue;
        } else if (CM.Cache[target][i].pp === CM.Cache.min) {
            color = CM.Disp.colorGreen;
        } else if (CM.Cache[target][i].pp === CM.Cache.max) {
            color = CM.Disp.colorRed;
        } else if (CM.Cache[target][i].pp > CM.Cache.max) {
            color = CM.Disp.colorPurple;
        } else if (CM.Cache[target][i].pp > CM.Cache.mid) {
            color = CM.Disp.colorOrange;
        } else {
            color = CM.Disp.colorYellow;
        }
...
CM.Cache.RemakeGoldenAndWrathCookiesMults = () => {
    ...
    if (CM.Sim.Has('Green yeast digestives')) {
        mult *= 1.01;
    }
    if (CM.Sim.Has('Dragon fang')) {
        mult *= 1.03;
    }
    ...
    // Calculate Dragon's Fortune multiplier adjustment: if Dragon's Fortune (or Reality Bending) aura is active and there are currently no golden cookies, compute a multiplier adjustment to apply on the current CPS to simulate 1 golden cookie on screen. Otherwise, the aura effect will be factored in the base CPS making the multiplier not requiring adjustment.
    CM.Cache.DragonsFortuneMultAdjustment = 1;
    if (Game.shimmerTypes.golden.n === 0) {
        CM.Cache.DragonsFortuneMultAdjustment *=
            1 + CM.Sim.auraMult("Dragon's Fortune") * 1.23;
    }
};

CM.Cache.MaxChainMoni = (digit, maxPayout, mult) => {
    let chain =
        1 + Math.max(0, Math.ceil(Math.log(Game.cookies) / Math.LN10) - 10);
    let moni = Math.max(
        digit,
        Math.min(Math.floor((1 / 9) * 10 ** chain * digit * mult), maxPayout),
    );
    let nextMoni = Math.max(
        digit,
        Math.min(
...
CM.Cache.RemakeChain = () => {
    let maxPayout = CM.Cache.NoGoldSwitchCookiesPS * 60 * 60 * 6;
    ...
    if (maxPayout < CM.Cache.ChainWrathReward) {
        ...
        CM.Cache.ChainWrath =
            CM.Cache.NextNumber(CM.Cache.ChainWrathReward) / 0.5;
    }

    CM.Cache.ChainFrenzyReward = CM.Cache.MaxChainMoni(
        7,
        maxPayout * 7,
        goldenMult,
    );

    CM.Cache.ChainFrenzyWrathReward = CM.Cache.MaxChainMoni(
        6,
        maxPayout * 7,
        wrathMult,
    );
    ...
    if (maxPayout * 7 < CM.Cache.ChainFrenzyReward) {
        ...
        CM.Cache.ChainFrenzy =
            CM.Cache.NextNumber(CM.Cache.ChainFrenzyReward) / 0.5;
    }
    if (maxPayout * 7 < CM.Cache.ChainFrenzyWrathReward) {
        CM.Cache.ChainFrenzyWrath = 0;
    } else {
        CM.Cache.ChainFrenzyWrath =
            CM.Cache.NextNumber(CM.Cache.ChainFrenzyWrathReward) / 0.5;
    }
};
...
CM.Config.LoadConfig = settings => {
    ...
    if (settings != null) {
            ...
            if (typeof CM.Options[i] === 'undefined') {
                ...
                if (i.indexOf('SoundURL') === -1) {
                    if (
                        !(
                            CM.Options[i] > -1 &&
                            CM.Options[i] < CM.ConfigData[i].label.length
                        )
                    ) {
                        mod = true;
                        CM.Options[i] = CM.Data.ConfigDefault[i];
                    }
                } else if (typeof CM.Options[i] !== 'string') {
                    // Sound URLs
                    mod = true;
                    CM.Options[i] = CM.Data.ConfigDefault[i];
                }
                ...
                for (const j in CM.Data.ConfigDefault.Header) {
                    if (
                        typeof CM.Options[i][j] === 'undefined' ||
                        !(CM.Options[i][j] > -1 && CM.Options[i][j] < 2)
                    ) {
                        mod = true;
        ...
        for (const i in CM.Data.ConfigDefault) {
            if (
                i !== 'Header' &&
                typeof CM.ConfigData[i].func !== 'undefined'
            ) {
...
CM.Config.ToggleConfig = config => {
    ...
    l(CM.ConfigPrefix + config).innerHTML =
        CM.ConfigData[config].label[CM.Options[config]];
    CM.Config.SaveConfig();
};
...
/**
 ...
 @param 	{string}	config	The name of the option
 */
CM.Config.ToggleConfigVolume = config => {
 ...
 */
CM.Config.ToggleHeader = config => {
    CM.Options.Header[config] += 1;
    if (CM.Options.Header[config] > 1) {
        CM.Options.Header[config] = 0;
    }
    CM.Config.SaveConfig();
};
 ...
 */
CM.Config.CheckNotificationPermissions = ToggleOnOff => {
    if (ToggleOnOff === 1) {
 ...
 */
// Barscolors
CM.ConfigData.BotBar = {
    type: 'bool',
    group: 'BarsColors',
    label: ['Bottom Bar OFF', 'Bottom Bar ON'],
    desc: 'Building Information',
    toggle: true,
    func: () => {
        CM.Disp.ToggleBotBar();
    },
};
CM.ConfigData.TimerBar = {
    type: 'bool',
    group: 'BarsColors',
    label: ['Timer Bar OFF', 'Timer Bar ON'],
    desc:
...
CM.ConfigData.TimerBarPos = {
    type: 'bool',
    group: 'BarsColors',
    label: ['Timer Bar Position (Top Left)', 'Timer Bar Position (Bottom)'],
    desc: 'Placement of the Timer Bar',
    toggle: false,
    func: () => {
        CM.Disp.ToggleTimerBarPos();
    },
};
CM.ConfigData.TimerBarOverlay = {
    type: 'bool',
    group: 'BarsColors',
    label: [
    ...
    desc: 'Overlay on timers displaying seconds and/or percentage left',
    toggle: true,
};
CM.ConfigData.SortBuildings = {
    type: 'bool',
    group: 'BarsColors',
    label: ['Sort Buildings: Default', 'Sort Buildings: PP'],
    desc: 'Sort the display of buildings in either default order or by PP',
    toggle: false,
    func: () => {
        CM.Disp.UpdateBuildings();
    },
};
CM.ConfigData.SortUpgrades = {
    type: 'bool',
    group: 'BarsColors',
    label: ['Sort Upgrades: Default', 'Sort Upgrades: PP'],
    desc: 'Sort the display of upgrades in either default order or by PP',
    toggle: false,
    func: () => {
        CM.Disp.UpdateUpgrades();
    },
};
CM.ConfigData.BuildColor = {
    type: 'bool',
    group: 'BarsColors',
    label: ['Building Colors OFF', 'Building Colors ON'],
    desc: 'Color code buildings',
    toggle: true,
    func: () => {
        CM.Disp.UpdateBuildings();
    },
};
CM.ConfigData.BulkBuildColor = {
    type: 'bool',
    group: 'BarsColors',
    label: [
...
CM.ConfigData.UpBarColor = {
    ...
    desc: 'Color code upgrades and optionally add a counter bar',
    toggle: false,
    func: () => {
...
CM.ConfigData.UpgradeBarFixedPos = {
    type: 'bool',
    group: 'BarsColors',
    label: ['Upgrade Bar Fixed Position OFF', 'Upgrade Bar Fixed Position ON'],
    desc:
        'Lock the upgrade bar at top of the screen to prevent it from moving ofscreen when scrolling',
    toggle: true,
    func: () => {
        CM.Disp.ToggleUpgradeBarFixedPos();
    },
};

// Calculation
CM.ConfigData.CalcWrink = {
    type: 'bool',
    group: 'Calculation',
    label: [
...
CM.ConfigData.CPSMode = {
    type: 'bool',
    group: 'Calculation',
    label: ['Current Cookies Per Second', 'Average Cookies Per Second'],
    desc:
        'Calculate times using current Cookies Per Second or average Cookies Per Second',
    toggle: false,
};
CM.ConfigData.AvgCPSHist = {
    type: 'bool',
    group: 'Calculation',
    label: [
    ...
    desc: 'How much time average Cookies Per Second should consider',
    toggle: false,
};
CM.ConfigData.AvgClicksHist = {
    type: 'bool',
    group: 'Calculation',
    label: [
    ...
    desc: 'How much time average Cookie Clicks should consider',
    toggle: false,
};
CM.ConfigData.ToolWarnBon = {
    type: 'bool',
    group: 'Calculation',
    label: [
...
// Notification
CM.ConfigData.GCNotification = {
    type: 'bool',
    group: 'NotificationGC',
    label: ['Notification OFF', 'Notification ON'],
    desc: 'Create a notification when Golden Cookie spawns',
    toggle: true,
    func: () => {
        CM.Config.CheckNotificationPermissions(CM.Options.GCNotification);
    },
};
CM.ConfigData.GCFlash = {
    type: 'bool',
    group: 'NotificationGC',
    label: ['Flash OFF', 'Flash ON'],
    desc: 'Flash screen on Golden Cookie',
    toggle: true,
};
CM.ConfigData.GCSound = {
    type: 'bool',
    group: 'NotificationGC',
    label: ['Sound OFF', 'Sound ON'],
    desc: 'Play a sound on Golden Cookie',
    toggle: true,
};
CM.ConfigData.GCVolume = {
    type: 'vol',
    group: 'NotificationGC',
    label: [],
    desc: 'Volume',
...
CM.ConfigData.GCSoundURL = {
    type: 'url',
    group: 'NotificationGC',
    label: 'Sound URL:',
    desc: 'URL of the sound to be played when a Golden Cookie spawns',
};
CM.ConfigData.FortuneNotification = {
    type: 'bool',
    group: 'NotificationFC',
    label: ['Notification OFF', 'Notification ON'],
    desc: 'Create a notification when Fortune Cookie is on the Ticker',
    toggle: true,
    func: () => {
        CM.Config.CheckNotificationPermissions(CM.Options.FortuneNotification);
    },
};
CM.ConfigData.FortuneFlash = {
    type: 'bool',
    group: 'NotificationFC',
    label: ['Flash OFF', 'Flash ON'],
    desc: 'Flash screen on Fortune Cookie',
    toggle: true,
};
CM.ConfigData.FortuneSound = {
    type: 'bool',
    group: 'NotificationFC',
    label: ['Sound OFF', 'Sound ON'],
    desc: 'Play a sound on Fortune Cookie',
    toggle: true,
};
CM.ConfigData.FortuneVolume = {
    type: 'vol',
    group: 'NotificationFC',
    label: [],
    desc: 'Volume',
...
CM.ConfigData.FortuneSoundURL = {
    type: 'url',
    group: 'NotificationFC',
    label: 'Sound URL:',
    desc: 'URL of the sound to be played when the Ticker has a Fortune Cookie',
};
CM.ConfigData.SeaNotification = {
    type: 'bool',
    group: 'NotificationSea',
    label: ['Notification OFF', 'Notification ON'],
    desc: 'Create a notification on Season Popup',
    toggle: true,
    func: () => {
        CM.Config.CheckNotificationPermissions(CM.Options.SeaNotification);
    },
};
CM.ConfigData.SeaFlash = {
    type: 'bool',
    group: 'NotificationSea',
    label: ['Flash OFF', 'Flash ON'],
    desc: 'Flash screen on Season Popup',
    toggle: true,
};
CM.ConfigData.SeaSound = {
    type: 'bool',
    group: 'NotificationSea',
    label: ['Sound OFF', 'Sound ON'],
    desc: 'Play a sound on Season Popup',
    toggle: true,
};
CM.ConfigData.SeaVolume = {
    type: 'vol',
    group: 'NotificationSea',
    label: [],
    desc: 'Volume',
...
CM.ConfigData.SeaSoundURL = {
    type: 'url',
    group: 'NotificationSea',
    label: 'Sound URL:',
    desc: 'URL of the sound to be played when a Season Special spawns',
};
CM.ConfigData.GardFlash = {
    type: 'bool',
    group: 'NotificationGard',
    label: ['Garden Tick Flash OFF', 'Flash ON'],
    desc: 'Flash screen on Garden Tick',
    toggle: true,
};
CM.ConfigData.GardSound = {
    type: 'bool',
    group: 'NotificationGard',
    label: ['Sound OFF', 'Sound ON'],
    desc: 'Play a sound on Garden Tick',
    toggle: true,
};
CM.ConfigData.GardVolume = {
    type: 'vol',
    group: 'NotificationGard',
    label: [],
    desc: 'Volume',
...
CM.ConfigData.GardSoundURL = {
    type: 'url',
    group: 'NotificationGard',
    label: 'Garden Tick Sound URL:',
    desc: 'URL of the sound to be played when the garden ticks',
};
CM.ConfigData.MagicNotification = {
    type: 'bool',
    group: 'NotificationMagi',
    label: ['Notification OFF', 'Notification ON'],
    desc: 'Create a notification when magic reaches maximum',
    toggle: true,
    func: () => {
        CM.Config.CheckNotificationPermissions(CM.Options.MagicNotification);
    },
};
CM.ConfigData.MagicFlash = {
    type: 'bool',
    group: 'NotificationMagi',
    label: ['Flash OFF', 'Flash ON'],
    desc: 'Flash screen when magic reaches maximum',
    toggle: true,
};
CM.ConfigData.MagicSound = {
    type: 'bool',
    group: 'NotificationMagi',
    label: ['Sound OFF', 'Sound ON'],
    desc: 'Play a sound when magic reaches maximum',
    toggle: true,
};
CM.ConfigData.MagicVolume = {
    type: 'vol',
    group: 'NotificationMagi',
    label: [],
    desc: 'Volume',
...
CM.ConfigData.MagicSoundURL = {
    ...
    group: 'NotificationMagi',
    label: 'Sound URL:',
    desc: 'URL of the sound to be played when magic reaches maxium',
};
CM.ConfigData.WrinklerNotification = {
    type: 'bool',
    group: 'NotificationWrink',
    label: ['Notification OFF', 'Notification ON'],
    desc: 'Create a notification when a Wrinkler appears',
    toggle: true,
    func: () => {
        CM.Config.CheckNotificationPermissions(CM.Options.WrinklerNotification);
    },
};
CM.ConfigData.WrinklerFlash = {
    type: 'bool',
    group: 'NotificationWrink',
    label: ['Flash OFF', 'Flash ON'],
    desc: 'Flash screen when a Wrinkler appears',
    toggle: true,
};
CM.ConfigData.WrinklerSound = {
    type: 'bool',
    group: 'NotificationWrink',
    label: ['Sound OFF', 'Sound ON'],
    desc: 'Play a sound when a Wrinkler appears',
    toggle: true,
};
CM.ConfigData.WrinklerVolume = {
    type: 'vol',
    group: 'NotificationWrink',
    label: [],
    desc: 'Volume',
...
CM.ConfigData.WrinklerSoundURL = {
    type: 'url',
    group: 'NotificationWrink',
    label: 'Sound URL:',
    desc: 'URL of the sound to be played when a Wrinkler appears',
};
CM.ConfigData.WrinklerMaxNotification = {
    type: 'bool',
    group: 'NotificationWrinkMax',
    label: ['Notification OFF', 'Notification ON'],
    desc:
        'Create a notification when the maximum amount of Wrinklers has appeared',
    toggle: true,
    func: () => {
        CM.Config.CheckNotificationPermissions(
            CM.Options.WrinklerMaxNotification,
        );
    },
};
CM.ConfigData.WrinklerMaxFlash = {
    type: 'bool',
    group: 'NotificationWrinkMax',
    label: ['Flash OFF', 'Flash ON'],
    desc: 'Flash screen when the maximum amount of Wrinklers has appeared',
    toggle: true,
};
CM.ConfigData.WrinklerMaxSound = {
    type: 'bool',
    group: 'NotificationWrinkMax',
    label: ['Sound OFF', 'Sound ON'],
    desc: 'Play a sound when the maximum amount of Wrinklers has appeared',
    toggle: true,
};
CM.ConfigData.WrinklerMaxVolume = {
    type: 'vol',
    group: 'NotificationWrinkMax',
    label: [],
    desc: 'Volume',
...
CM.ConfigData.WrinklerMaxSoundURL = {
    type: 'url',
    group: 'NotificationWrinkMax',
    label: 'Sound URL:',
    desc:
...
CM.ConfigData.Title = {
    type: 'bool',
    group: 'Notification',
    label: ['Title OFF', 'Title ON', 'Title Pinned Tab Highlight'],
    desc:
        'Update title with Golden Cookie/Season Popup timers; pinned tab highlight only changes the title when a Golden Cookie/Season Popup spawns',
    toggle: true,
};

// Tooltip
CM.ConfigData.TooltipBuildUpgrade = {
    type: 'bool',
    group: 'Tooltip',
    label: [
    ...
    desc: 'Extra information in Building/Upgrade tooltips',
    toggle: true,
};
CM.ConfigData.TooltipAmor = {
    type: 'bool',
    group: 'Tooltip',
    label: [
    ...
    desc: 'Add amortization information to buildings tooltip',
    toggle: true,
};
CM.ConfigData.ToolWarnLucky = {
    type: 'bool',
    group: 'Tooltip',
    label: ['Tooltip Lucky Warning OFF', 'Tooltip Lucky Warning ON'],
    desc:
        'A warning when buying if it will put the bank under the amount needed for max "Lucky!" rewards',
    toggle: true,
};
CM.ConfigData.ToolWarnLuckyFrenzy = {
    type: 'bool',
    group: 'Tooltip',
    label: [
        'Tooltip Lucky Frenzy Warning OFF',
        'Tooltip Lucky Frenzy Warning ON',
    ],
    desc:
        'A warning when buying if it will put the bank under the amount needed for max "Lucky!" (Frenzy) rewards',
    toggle: true,
};
CM.ConfigData.ToolWarnConjure = {
    type: 'bool',
    group: 'Tooltip',
    label: ['Tooltip Conjure Warning OFF', 'Tooltip Conjure Warning ON'],
    desc:
        'A warning when buying if it will put the bank under the amount needed for max "Conjure Baked Goods" rewards',
    toggle: true,
};
CM.ConfigData.ToolWarnConjureFrenzy = {
    type: 'bool',
    group: 'Tooltip',
    label: [
        'Tooltip Conjure Frenzy Warning OFF',
        'Tooltip Conjure Frenzy Warning ON',
    ],
    desc:
...
CM.ConfigData.ToolWarnEdifice = {
    type: 'bool',
    group: 'Tooltip',
    label: ['Tooltip Edifice Warning OFF', 'Tooltip Edifice Warning ON'],
    desc:
...
CM.ConfigData.ToolWarnPos = {
    type: 'bool',
    group: 'Tooltip',
    label: [
        'Tooltip Warning Position (Left)',
        'Tooltip Warning Position (Bottom)',
    ],
    desc: 'Placement of the warning boxes',
    toggle: false,
    func: () => {
...
CM.ConfigData.TooltipGrim = {
    ...
    group: 'Tooltip',
    label: [
        'Grimoire Tooltip Information OFF',
        'Grimoire Tooltip Information ON',
    ],
    desc: 'Extra information in tooltip for grimoire',
    toggle: true,
};
CM.ConfigData.TooltipWrink = {
    type: 'bool',
    group: 'Tooltip',
    label: ['Wrinkler Tooltip OFF', 'Wrinkler Tooltip ON'],
    desc: 'Shows the amount of cookies a wrinkler will give when popping it',
    toggle: true,
};
CM.ConfigData.TooltipLump = {
    type: 'bool',
    group: 'Tooltip',
    label: ['Sugar Lump Tooltip OFF', 'Sugar Lump Tooltip ON'],
    desc: 'Shows the current Sugar Lump type in Sugar lump tooltip.',
    toggle: true,
};
CM.ConfigData.TooltipPlots = {
    type: 'bool',
    group: 'Tooltip',
    label: ['Garden Plots Tooltip OFF', 'Garden Plots Tooltip ON'],
    desc: 'Shows a tooltip for plants that have a cookie reward.',
    toggle: true,
};
CM.ConfigData.DragonAuraInfo = {
    type: 'bool',
    group: 'Tooltip',
    label: ['Extra Dragon Aura Info OFF', 'Extra Dragon Aura Info ON'],
    desc:
...
CM.ConfigData.Stats = {
    type: 'bool',
    group: 'Statistics',
    label: ['Statistics OFF', 'Statistics ON'],
    desc: 'Extra Cookie Monster statistics!',
    toggle: true,
};
CM.ConfigData.MissingUpgrades = {
    type: 'bool',
    group: 'Statistics',
    label: ['Missing Upgrades OFF', 'Missing Upgrades ON'],
    desc:
        'Shows Missing upgrades in Stats Menu. This feature can be laggy for users with a low amount of unlocked achievements.',
    toggle: true,
};
CM.ConfigData.UpStats = {
    type: 'bool',
    group: 'Statistics',
    label: ['Statistics Update Rate (Default)', 'Statistics Update Rate (1s)'],
    desc: 'Default Game rate is once every 5 seconds',
    toggle: false,
};
CM.ConfigData.TimeFormat = {
    type: 'bool',
    group: 'Statistics',
    label: ['Time XXd, XXh, XXm, XXs', 'Time XX:XX:XX:XX:XX'],
    desc: 'Change the time format',
    toggle: false,
};
CM.ConfigData.DetailedTime = {
    type: 'bool',
    group: 'Statistics',
    label: ['Detailed Time OFF', 'Detailed Time ON'],
    desc: 'Change how time is displayed in certain statistics and tooltips',
    toggle: true,
    func: () => {
        CM.Disp.ToggleDetailedTime();
    },
};
CM.ConfigData.GrimoireBar = {
    type: 'bool',
    group: 'Statistics',
    label: ['Grimoire Magic Meter Timer OFF', 'Grimoire Magic Meter Timer ON'],
    desc: 'A timer on how long before the Grimoire magic meter is full',
    toggle: true,
};
...
CM.ConfigData.Scale = {
    ...
    desc: 'Change how long numbers are handled',
    toggle: false,
    func: () => {
        CM.Disp.RefreshScale();
    },
};
CM.ConfigData.ScaleDecimals = {
    type: 'bool',
    group: 'Notation',
    label: ['1 decimals', '2 decimals', '3 decimals'],
    desc: 'Set the number of decimals used when applicable',
    toggle: false,
    func: () => {
        CM.Disp.RefreshScale();
    },
};
CM.ConfigData.ScaleSeparator = {
    type: 'bool',
    group: 'Notation',
    label: ['. for decimals (Standard)', '. for thousands'],
    desc: 'Set the separator used for decimals and thousands',
    toggle: false,
    func: () => {
        CM.Disp.RefreshScale();
    },
};
CM.ConfigData.ScaleCutoff = {
    type: 'numscale',
    group: 'Notation',
    label: 'Notation Cut-off Point',
    desc:
...
// Miscellaneous
CM.ConfigData.GCTimer = {
    type: 'bool',
    group: 'Miscellaneous',
    label: ['Golden Cookie Timer OFF', 'Golden Cookie Timer ON'],
    desc: 'A timer on the Golden Cookie when it has been spawned',
    toggle: true,
    func: () => {
        CM.Disp.ToggleGCTimer();
    },
};
CM.ConfigData.Favicon = {
    type: 'bool',
    group: 'Miscellaneous',
    label: ['Favicon OFF', 'Favicon ON'],
    desc: 'Update favicon with Golden/Wrath Cookie',
    toggle: true,
    func: () => {
        CM.Disp.UpdateFavicon();
    },
};
CM.ConfigData.WrinklerButtons = {
    type: 'bool',
    group: 'Miscellaneous',
    label: ['Extra Buttons OFF', 'Extra Buttons ON'],
    desc: 'Show buttons for popping wrinklers at bottom of cookie section',
    toggle: true,
    func: () => {
        CM.Disp.UpdateWrinklerButtons();
    },
};
CM.ConfigData.BulkBuyBlock = {
    type: 'bool',
    group: 'Miscellaneous',
    label: ['Block Bulk Buying OFF', 'Block Bulk Buying ON'],
    desc:
...
/*
 Section: Auxiliary functions used by other functions
 */
...
CM.Disp.GetCPS = () => {
    ...
    if (CM.Options.CalcWrink === 1) {
        return (
            Game.cookiesPs *
            (CM.Cache.CurrWrinklerCPSMult +
                (1 - CM.Cache.CurrWrinklerCount * 0.05))
        );
    }
    if (CM.Options.CalcWrink === 2) {
        ...
        if (Game.wrinklers[CM.Cache.WrinklersFattest[1]].type === 1) {
            return (
                Game.cookiesPs *
                ((CM.Cache.CurrWrinklerCPSMult * 3) /
                    CM.Cache.CurrWrinklerCount +
                    (1 - CM.Cache.CurrWrinklerCount * 0.05))
            );
        }
        return (
            Game.cookiesPs *
            (CM.Cache.CurrWrinklerCPSMult / CM.Cache.CurrWrinklerCount +
                (1 - CM.Cache.CurrWrinklerCount * 0.05))
        );
    }
};
 ...
 */
CM.Disp.CalculateGrimoireRefillTime = (currentMagic, maxMagic, targetMagic) => {
    let count = 0;
    while (currentMagic < targetMagic) {
        currentMagic +=
            Math.max(0.002, currentMagic / Math.max(maxMagic, 100) ** 0.5) *
            0.002;
        count += 1;
    }
 ...
 */
CM.Disp.FormatTime = (time, longFormat) => {
    if (time === Infinity) {
        return time;
    }
    time = Math.ceil(time);
 ...
 */
CM.Disp.Beautify = (num, floats, forced) => {
    ...
    if (Number.isFinite(num)) {
        ...
        if (num > 0.001 && num < CM.Options.ScaleCutoff) {
        ...
        if ((CM.Options.Scale === 4 && !forced) || forced === 4) {
            ...
            if ((CM.Options.Scale === 1 && !forced) || forced === 1) {
                // Metric scale, 123456789 => 123.457 M
                if (
                    num >= 0.01 &&
                    num < Number(`1e${CM.Data.metric.length * 3}`)
                ) {
                    answer += ` ${CM.Data.metric[AmountOfTenPowerThree]}`;
                }
                // If number is too large or little, revert to scientific notation
                else {
                    answer = CM.Disp.Beautify(num, 0, 4);
                }
            } else if ((CM.Options.Scale === 2 && !forced) || forced === 2) {
                // Short scale, 123456789 => 123.457 M
                if (
                    num >= 0.01 &&
                    num < Number(`1e${CM.Data.shortScale.length * 3}`)
                ) {
            ...
            } else if ((CM.Options.Scale === 3 && !forced) || forced === 3) {
                // Short scale, 123456789 => 123.457 M
                if (
                    num >= 0.01 &&
                    num <
                        Number(`1e${CM.Data.shortScaleAbbreviated.length * 3}`)
                ) {
            ...
            } else if ((CM.Options.Scale === 5 && !forced) || forced === 5) {
        ...
        if (answer === '') {
            console.log(
                `Could not beautify number with CM.Disp.Beautify:${num}`,
            );
            answer = CM.Backup.Beautify(num, floats);
            answer = CM.Backup.Beautify(num, floats);
            answer = CM.Backup.Beautify(num, floats);
        }
        if (CM.Options.ScaleSeparator) {
...
CM.Disp.Draw = () => {
    ...
    if (
        ...
        Game.onMenu === 'stats' &&
        CM.Options.Stats // while being on the stats menu only
    ) {
...
CM.Disp.CreateBotBar = () => {
    ...
    CM.Disp.BotBar.style.backgroundImage =
        'linear-gradient(to bottom, #4d4548, #000000)';
    CM.Disp.BotBar.style.borderTop = '1px solid black';
    CM.Disp.BotBar.style.overflow = 'auto';
    CM.Disp.BotBar.style.textShadow =
    ...
    type.appendChild(
        firstCol(
            `CM ${CM.VersionMajor}.${CM.VersionMinor}`,
            CM.Disp.colorYellow,
        ),
    );
...
CM.Disp.UpdateBotBar = () => {
    if (CM.Options.BotBar === 1 && CM.Cache.Objects) {
        ...
        for (const i of Object.keys(CM.Cache.Objects)) {
            ...
            count += 1;
            CM.Disp.BotBar.firstChild.firstChild.childNodes[0].childNodes[
                count
            ].childNodes[1].textContent = Game.Objects[i].amount;
            CM.Disp.BotBar.firstChild.firstChild.childNodes[1].childNodes[
                count
            ].textContent = Beautify(CM.Cache[target][i].bonus, 2);
            CM.Disp.BotBar.firstChild.firstChild.childNodes[2].childNodes[
                count
            ].className = CM.Disp.colorTextPre + CM.Cache[target][i].color;
            CM.Disp.BotBar.firstChild.firstChild.childNodes[2].childNodes[
                count
            ].textContent = Beautify(CM.Cache[target][i].pp, 2);
            const timeColor = CM.Disp.GetTimeColor(
                (Game.Objects[i].bulkPrice -
                    (Game.cookies + CM.Disp.GetWrinkConfigBank())) /
                    CM.Disp.GetCPS(),
            );
            CM.Disp.BotBar.firstChild.firstChild.childNodes[3].childNodes[
                count
            ].className = CM.Disp.colorTextPre + timeColor.color;
            if (
                timeColor.text === 'Done!' &&
 ...
 */
CM.Disp.CreateBotBarBuildingColumn = buildingName => {
    if (!CM.Disp.BotBar) {
        CM.Disp.CreateBotBar();
 ...
 */
CM.Disp.TimerBarCreateBar = (id, name, bars) => {
...
CM.Disp.CreateTimerBar = () => {
    ...
    CM.Disp.TimerBars.CMTimerBarGC = CM.Disp.TimerBarCreateBar(
        'CMTimerBarGC',
        'Next Cookie',
        [
            { id: 'CMTimerBarGCMinBar', color: CM.Disp.colorGray },
            { id: 'CMTimerBarGCBar', color: CM.Disp.colorPurple },
        ],
    );
    CM.Disp.TimerBar.appendChild(CM.Disp.TimerBars.CMTimerBarGC);

    // Create standard Reindeer bar
    CM.Disp.TimerBars.CMTimerBarRen = CM.Disp.TimerBarCreateBar(
        'CMTimerBarRen',
        'Next Reindeer',
        [
            { id: 'CMTimerBarRenMinBar', color: CM.Disp.colorGray },
            { id: 'CMTimerBarRenBar', color: CM.Disp.colorOrange },
        ],
    );
...
CM.Disp.UpdateTimerBar = () => {
    if (CM.Options.TimerBar === 1) {
        ...
        if (
            ...
            if (CM.Options.TimerBarOverlay >= 1) {
            ...
            CM.Disp.TimerBars.CMTimerBarGC.style.display = 'none';
        }
        ...
        for (const i of Object.keys(Game.buffs)) {
            if (Game.buffs[i]) {
                const timer = CM.Disp.TimerBarCreateBar(
                    Game.buffs[i].name,
                    Game.buffs[i].name,
                ...
                if (
                    typeof CM.Disp.buffColors[Game.buffs[i].name] !==
                    'undefined'
                ) {
...
CM.Disp.UpdateBotTimerBarPosition = () => {
    if (
        CM.Options.BotBar === 1 &&
        CM.Options.TimerBar === 1 &&
        CM.Options.TimerBarPos === 1
    ) {
        CM.Disp.BotBar.style.bottom = CM.Disp.TimerBar.style.height;
        l('game').style.bottom = `${
            Number(CM.Disp.TimerBar.style.height.replace('px', '')) + 70
        }px`;
...
CM.Disp.UpdateBuildings = () => {
    let target = 'Objects';
    if (Game.buyBulk === 10 && CM.Options.BulkBuildColor === 1) {
        target = 'Objects10';
    } else if (Game.buyBulk === 100 && CM.Options.BulkBuildColor === 1) {
        target = 'Objects100';
    }
    if (Game.buyMode === 1) {
        if (CM.Options.BuildColor === 1) {
            for (const i of Object.keys(CM.Cache[target])) {
                l(`productPrice${Game.Objects[i].id}`).style.color =
                    CM.Options.Colors[CM.Cache[target][i].color];
            }
        } else {
            for (const i of Object.keys(Game.Objects)) {
                l(`productPrice${Game.Objects[i].id}`).style.removeProperty(
                    'color',
                );
            }
        }
    ...
    if (Game.buyMode === 1 && CM.Options.SortBuildings) {
        ...
        const arr = Object.keys(CM.Cache.Objects).map(k => {
            const o = CM.Cache.Objects[k];
            o.name = k;
            o.id = Game.Objects[k].id;
            return o;
        });
...
CM.Disp.UpdateUpgrades = () => {
    ...
    if (CM.Options.UpBarColor > 0) {
        ...
        for (const i of Object.keys(Game.UpgradesInStore)) {
            ...
            for (let j = 0; j < l(`upgrade${i}`).childNodes.length; j++) {
                if (
                    l(`upgrade${i}`).childNodes[j].className.indexOf(
                        CM.Disp.colorBackPre,
                    ) !== -1
                ) {
                    l(`upgrade${i}`).childNodes[j].className =
                        CM.Disp.colorBackPre + CM.Cache.Upgrades[me.name].color;
                    addedColor = true;
                    break;
                }
            }
            ...
            if (CM.Cache.Upgrades[me.name].color === CM.Disp.colorBlue) {
                blue += 1;
            } else if (
                CM.Cache.Upgrades[me.name].color === CM.Disp.colorGreen
            ) {
                green += 1;
            } else if (
                CM.Cache.Upgrades[me.name].color === CM.Disp.colorYellow
            ) {
                yellow += 1;
            } else if (
                CM.Cache.Upgrades[me.name].color === CM.Disp.colorOrange
            ) {
                orange += 1;
            } else if (CM.Cache.Upgrades[me.name].color === CM.Disp.colorRed) {
                red += 1;
            } else if (
                CM.Cache.Upgrades[me.name].color === CM.Disp.colorPurple
            ) {
                purple += 1;
            } else if (CM.Cache.Upgrades[me.name].color === CM.Disp.colorGray) {
                gray += 1;
            }
        }
    ...
    const nameChecker = (array, upgrade) => {
        return array.findIndex(e => e.name === upgrade.name);
    };
    for (let x = 0; x < Game.UpgradesInStore.length; x++) {
 ...
 */
CM.Disp.ToggleUpgradeBarAndColor = () => {
    if (CM.Options.UpBarColor === 1) {
        // Colours and bar on
        CM.Disp.UpgradeBar.style.display = '';
        CM.Disp.UpdateUpgrades();
    } else if (CM.Options.UpBarColor === 2) {
 ...
 */
CM.Disp.ToggleUpgradeBarFixedPos = () => {
    if (CM.Options.UpgradeBarFixedPos === 1) {
...
CM.Disp.CreateUpgradeBar = () => {
    ...
    CM.Disp.UpgradeBar.appendChild(
        upgradeNumber('CMUpgradeBarBlue', CM.Disp.colorBlue),
    );
    CM.Disp.UpgradeBar.appendChild(
        upgradeNumber('CMUpgradeBarGreen', CM.Disp.colorGreen),
    );
    CM.Disp.UpgradeBar.appendChild(
        upgradeNumber('CMUpgradeBarYellow', CM.Disp.colorYellow),
    );
    CM.Disp.UpgradeBar.appendChild(
        upgradeNumber('CMUpgradeBarOrange', CM.Disp.colorOrange),
    );
    CM.Disp.UpgradeBar.appendChild(
        upgradeNumber('CMUpgradeBarRed', CM.Disp.colorRed),
    );
    CM.Disp.UpgradeBar.appendChild(
        upgradeNumber('CMUpgradeBarPurple', CM.Disp.colorPurple),
    );
    CM.Disp.UpgradeBar.appendChild(
        upgradeNumber('CMUpgradeBarGray', CM.Disp.colorGray),
    );
 ...
 */
CM.Disp.CreateUpgradeBarLegend = () => {
    ...
    legend.appendChild(
        legendLine(CM.Disp.colorBlue, 'Better than best PP building'),
    );
    legend.appendChild(
        legendLine(CM.Disp.colorGreen, 'Same as best PP building'),
    );
    legend.appendChild(
        legendLine(
            CM.Disp.colorYellow,
    ...
    legend.appendChild(
        legendLine(CM.Disp.colorRed, 'Same as worst PP building'),
    );
    legend.appendChild(
        legendLine(CM.Disp.colorPurple, 'Worse than worst PP building'),
    );
    legend.appendChild(
        legendLine(CM.Disp.colorGray, 'Negative or infinity PP'),
    );
    return legend;
};
...
CM.Disp.Flash = (mode, config) => {
    ...
    if ((CM.Options[config] === 1 && mode === 3) || mode === 1) {
        ...
        if (mode === 3) {
            ...
            setTimeout(() => {
                CM.Disp.Flash(2, config);
            }, 1000 / Game.fps);
        } else {
            setTimeout(() => {
                CM.Disp.Flash(0, config);
            }, 1000 / Game.fps);
        }
    } else if (mode === 2) {
        CM.Disp.WhiteScreen.style.opacity = '1';
        setTimeout(() => {
            CM.Disp.Flash(1, config);
        }, 1000 / Game.fps);
    } else if (mode === 0) {
        CM.Disp.WhiteScreen.style.display = 'none';
    }
 ...
 */
CM.Disp.PlaySound = (url, sndConfig, volConfig) => {
    // The arguments check makes the sound not play upon initialization of the mod
 ...
 */
CM.Disp.Notification = (notifyConfig, title, message) => {
    // The arguments check makes the sound not play upon initialization of the mod
    if (
        CM.Options[notifyConfig] === 1 &&
...
CM.Disp.UpdateFavicon = () => {
    if (CM.Options.Favicon === 1 && CM.Main.lastGoldenCookieState > 0) {
        if (CM.Cache.spawnedGoldenShimmer.wrath) {
            CM.Disp.Favicon.href =
...
CM.Disp.UpdateTitle = () => {
    if (Game.OnAscend || CM.Options.Title === 0) {
        ...
        if (CM.Cache.spawnedGoldenShimmer) {
            if (CM.Cache.spawnedGoldenShimmer.wrath) {
                titleGC = `[W ${Math.ceil(
                    CM.Cache.spawnedGoldenShimmer.life / Game.fps,
                )}]`;
            } else {
                titleGC = `[G ${Math.ceil(
                    CM.Cache.spawnedGoldenShimmer.life / Game.fps,
        ...
        if (Game.season === 'christmas') {
            addSP = true;
            if (CM.Main.lastSeasonPopupState) {
                titleSP = `[R ${Math.ceil(
                    CM.Cache.seasonPopShimmer.life / Game.fps,
                )}]`;
            } else {
                titleSP = `[${Math.ceil(
                    (Game.shimmerTypes.reindeer.maxTime -
...
CM.Disp.ToggleGCTimer = () => {
    if (CM.Options.GCTimer === 1) {
        for (const i of Object.keys(CM.Disp.GCTimers)) {
            ...
            CM.Disp.GCTimers[i].style.left =
                CM.Cache.goldenShimmersByID[i].l.style.left;
            CM.Disp.GCTimers[i].style.top =
                CM.Cache.goldenShimmersByID[i].l.style.top;
        }
    } else {
        for (const i of Object.keys(CM.Disp.GCTimers)) {
 ...
 */
CM.Disp.CreateSimpleTooltip = (placeholder, text, minWidth) => {
...
CM.Disp.Tooltip = (type, name) => {
                ...
                l('tooltip').innerHTML = l('tooltip')
                    ...
                    .join(
                        `so far<br/>&bull; <b>${Beautify(amortizeAmount)}</b> ${
                            Math.floor(amortizeAmount) === 1
                                ? 'cookie'
                                : 'cookies'
                        } left to amortize (${
                            CM.Disp.GetTimeColor(
                                (buildPrice - Game.Objects[name].totalCookies) /
                                    (Game.Objects[name].storedTotalCps *
                                        Game.globalCpsMult),
                            ).text
                        })</div>`,
                    );
    ...
    // Sugar Lumps
        ...
        l('tooltip').innerHTML = Game.Objects[
            'Wizard tower'
        ].minigame.spellTooltip(name)();
    }
    // Grimoire
    else if (type === 'p') {
 ...
 */
CM.Disp.TooltipCreateTooltipBox = () => {
 ...
 */
CM.Disp.TooltipCreateHeader = text => {
    const div = document.createElement('div');
 ...
 */
CM.Disp.TooltipCreateCalculationSection = tooltip => {
    ...
    if (CM.Disp.tooltipType === 'b') {
        tooltip.appendChild(
            CM.Disp.TooltipCreateHeader(
                'Production left till next achievement',
            ),
        );
 ...
 */
CM.Disp.TooltipCreateWarningSection = () => {
    ...
    const create = (boxId, color, labelTextFront, labelTextBack, deficitId) => {
    ...
    CM.Disp.TooltipWarn.appendChild(
        create(
...
CM.Disp.UpdateTooltipBuilding = () => {
    if (CM.Options.TooltipBuildUpgrade === 1 && Game.buyMode === 1) {
        ...
        CM.Disp.TooltipBonusIncome =
            CM.Cache[target][CM.Disp.tooltipName].bonus;

        if (CM.Options.TooltipBuildUpgrade === 1 && Game.buyMode === 1) {
            l('CMTooltipIncome').textContent = Beautify(
                CM.Disp.TooltipBonusIncome,
                2,
            );
            const increase = Math.round(
                (CM.Disp.TooltipBonusIncome / Game.cookiesPs) * 10000,
            );
            if (Number.isFinite(increase) && increase !== 0) {
                l('CMTooltipIncome').textContent += ` (${
                    increase / 100
                }% of income)`;
            }
            l('CMTooltipBorder').className =
                CM.Disp.colorTextPre +
                CM.Cache[target][CM.Disp.tooltipName].color;
            l('CMTooltipPP').textContent = Beautify(
                CM.Cache[target][CM.Disp.tooltipName].pp,
                2,
            );
            ...
            if (
                timeColor.text === 'Done!' &&
                Game.cookies < CM.Cache[target][CM.Disp.tooltipName].price
            ) {
                l(
                    'CMTooltipTime',
                ).textContent = `${timeColor.text} (with Wrink)`;
            ...
            l('CMTooltipTime').className =
                CM.Disp.colorTextPre + timeColor.color;
        }
        ...
        for (const i of Object.keys(
            Game.Objects[CM.Disp.tooltipName].productionAchievs,
        )) {
            if (
                !CM.Sim.HasAchiev(
                    Game.Objects[CM.Disp.tooltipName].productionAchievs[i]
                        .achiev.name,
                )
            ) {
                ...
                l(
                    'CMTooltipProduction',
                ).className = `ProdAchievement${CM.Disp.tooltipName}`;
                l('CMTooltipProduction').textContent = Beautify(
                    nextProductionAchiev.pow -
                        CM.Sim.Objects[CM.Disp.tooltipName].totalCookies,
                    15,
                );
...
CM.Disp.UpdateTooltipUpgrade = () => {
    ...
    CM.Disp.TooltipBonusIncome =
        CM.Cache.Upgrades[Game.UpgradesInStore[CM.Disp.tooltipName].name].bonus;
    CM.Disp.TooltipPrice = Game.Upgrades[
        Game.UpgradesInStore[CM.Disp.tooltipName].name
    ].getPrice();
    CM.Disp.TooltipBonusMouse =
        CM.Cache.Upgrades[
            Game.UpgradesInStore[CM.Disp.tooltipName].name
        ].bonusMouse;
    ...
    if (CM.Options.TooltipBuildUpgrade === 1) {
        ...
        l('CMTooltipBorder').className =
            CM.Disp.colorTextPre +
            CM.Cache.Upgrades[Game.UpgradesInStore[CM.Disp.tooltipName].name]
                .color;
        // If clicking power upgrade
        if (CM.Disp.TooltipBonusMouse) {
            l('CMTooltipCookiePerClick').textContent = Beautify(
                CM.Disp.TooltipBonusMouse,
            );
            l('CMTooltipCookiePerClick').style.display = 'block';
            l('CMTooltipCookiePerClick').previousSibling.style.display =
                'block';
        }
        ...
        if (CM.Disp.TooltipBonusIncome === 0 && CM.Disp.TooltipBonusMouse) {
            l('CMTooltipPP').textContent = `${Beautify(
                CM.Disp.TooltipPrice / CM.Disp.TooltipBonusMouse,
            )} Clicks`;
        ...
        if (
            timeColor.text === 'Done!' &&
            Game.cookies < Game.UpgradesInStore[CM.Disp.tooltipName].getPrice()
        ) {
        ...
        if (
            Game.UpgradesInStore[CM.Disp.tooltipName].name === 'Chocolate egg'
        ) {
            l('CMTooltipBorder').lastChild.style.marginBottom = '4px';
            l('CMTooltipBorder').appendChild(
                CM.Disp.TooltipCreateHeader(
                    'Cookies to be gained (Currently/Max)',
                ),
            );
            const chocolate = document.createElement('div');
            chocolate.style.color = 'white';
            chocolate.textContent = `${CM.Disp.Beautify(
                Game.cookies * 0.05,
            )} / ${CM.Disp.Beautify(CM.Cache.lastChoEgg)}`;
...
CM.Disp.UpdateTooltipSugarLump = () => {
    if (CM.Options.TooltipLump === 1) {
        ...
        tooltipBox.appendChild(
            CM.Disp.TooltipCreateHeader('Current Sugar Lump'),
        );
...
CM.Disp.UpdateTooltipGrimoire = () => {
    ...
    const spellCost = minigame.getSpellCost(
        minigame.spellsById[CM.Disp.tooltipName],
    );
    ...
    if (CM.Options.TooltipGrim === 1 && spellCost <= minigame.magicM) {
        ...
        const timeColor = CM.Disp.GetTimeColor(
            CM.Disp.CalculateGrimoireRefillTime(
                minigame.magic,
                minigame.magicM,
                spellCost,
            ),
        ...
        if (spellCost <= minigame.magic) {
            ...
            const recoverColor = CM.Disp.GetTimeColor(
                CM.Disp.CalculateGrimoireRefillTime(
        ...
        if (CM.Disp.tooltipName === 0) {
            tooltipBox.appendChild(
                CM.Disp.TooltipCreateHeader('Cookies to be gained/lost'),
            );
            ...
            reward.textContent = Beautify(
                Math.min(
                    (Game.cookies + CM.Disp.GetWrinkConfigBank()) * 0.15,
                    CM.Cache.NoGoldSwitchCookiesPS * 60 * 30,
                ),
                2,
            );
            ...
            loss.textContent = Beautify(
                CM.Cache.NoGoldSwitchCookiesPS * 60 * 15,
                2,
            );
...
CM.Disp.UpdateTooltipGardenPlots = () => {
    ...
    if (
        CM.Options.TooltipLump &&
        minigame.plot[CM.Disp.tooltipName[1]][CM.Disp.tooltipName[0]][0] !== 0
    ) {
        const mature =
            minigame.plot[CM.Disp.tooltipName[1]][CM.Disp.tooltipName[0]][1] >
            minigame.plantsById[
                minigame.plot[CM.Disp.tooltipName[1]][
                    CM.Disp.tooltipName[0]
                ][0] - 1
            ].matureBase;
        const plantName =
            minigame.plantsById[
        ...
        l('CMTooltipBorder').appendChild(
            CM.Disp.TooltipCreateHeader('Reward (Current / Maximum)'),
        );
        ...
        if (plantName === 'Bakeberry') {
            l('CMTooltipPlantReward').textContent = `${
                mature
                    ...
                    : '0'
            } / ${CM.Disp.Beautify(Game.cookiesPs * 60 * 30)}`;
        } else if (
            plantName === 'Chocoroot' ||
            ...
            l('CMTooltipPlantReward').textContent = `${
                mature
                    ...
                    : '0'
            } / ${CM.Disp.Beautify(Game.cookiesPs * 60 * 3)}`;
            ...
            l('CMTooltipPlantReward').textContent = `${
                mature
                    ...
                    : '0'
            } / ${CM.Disp.Beautify(Game.cookiesPs * 60 * 60)}`;
            ...
            l('CMTooltipPlantReward').textContent = `${
                mature
                    ...
                    : '0'
            } / ${CM.Disp.Beautify(Game.cookiesPs * 60 * 120)}`;
        } else {
            l('CMTooltipArea').style.display = 'none';
        }
    } else {
...
CM.Disp.UpdateTooltipHarvestAll = () => {
    ...
    if (CM.Options.TooltipLump) {
        l('CMTooltipBorder').appendChild(
            CM.Disp.TooltipCreateHeader('Cookies gained from harvesting:'),
        );
        let totalGain = 0;
        let mortal = 0;
        ...
        for (let y = 0; y < 6; y++) {
            for (let x = 0; x < 6; x++) {
                if (minigame.plot[y][x][0] >= 1) {
                    ...
                    let count = true;
                    if (mortal && me.immortal) {
                        count = false;
                    }
                    if (tile[1] < me.matureBase) {
                        count = false;
                    }
        ...
        l('CMTooltipBorder').appendChild(
            document.createTextNode(CM.Disp.Beautify(totalGain)),
        );
    } else {
        l('CMTooltipArea').style.display = 'none';
    }
...
CM.Disp.UpdateTooltipWarnings = () => {
    if (CM.Disp.tooltipType === 'b' || CM.Disp.tooltipType === 'u') {
        ...
        const amount =
            Game.cookies + CM.Disp.GetWrinkConfigBank() - CM.Disp.TooltipPrice;
        ...
        if (CM.Options.ToolWarnLucky === 1) {
            if (
                amount < limitLucky &&
                (CM.Disp.tooltipType !== 'b' || Game.buyMode === 1)
            ) {
                ...
                l('CMDispTooltipWarnLuckyText').textContent = `${Beautify(
                    ...
                    (limitLucky - amount) /
                        (CM.Disp.GetCPS() + CM.Disp.TooltipBonusIncome),
                )})`;
            } else {
...
CM.Disp.UpdateTooltipLocation = () => {
    if (Game.tooltip.origin === 'store') {
        let warnOffset = 0;
        if (
            CM.Options.ToolWarnLucky === 1 &&
            CM.Options.ToolWarnPos === 1 &&
            typeof CM.Disp.TooltipWarn !== 'undefined'
        ) {
    ...
    //     !Game.OnAscend &&
    //     CM.Options.TimerBar === 1 &&
    //     CM.Options.TimerBarPos === 0
    // ) {
    //     Game.tooltip.tta.style.top = `${
    //         parseInt(Game.tooltip.tta.style.top, 10) +
    //         parseInt(CM.Disp.TimerBar.style.height, 10)
    //     }px`;
    // }
};
 ...
 */
CM.Disp.CheckWrinklerTooltip = () => {
    if (CM.Options.TooltipWrink === 1 && CM.Disp.TooltipWrinklerArea === 1) {
        ...
        for (const i of Object.keys(Game.wrinklers)) {
            ...
            if (me.phase > 0 && me.selected) {
                ...
                if (
                    ...
                    CM.Disp.TooltipWrinklerBeingShown[i] === undefined
                ) {
                    const placeholder = document.createElement('div');
...
CM.Disp.UpdateWrinklerTooltip = () => {
    if (CM.Options.TooltipWrink === 1 && l('CMTooltipWrinkler') != null) {
        ...
        // Shiny wrinklers
        if (Game.wrinklers[CM.Disp.TooltipWrinkler].type === 1) {
            toSuck *= 3;
        }
        sucked *= toSuck;
        if (Game.Has('Wrinklerspawn')) {
...
CM.Disp.AddAuraInfo = aura => {
    if (CM.Options.DragonAuraInfo === 1) {
        ...
        const timeToRecover = CM.Disp.FormatTime(
    ...
    if (
        ...
        for (let i = 0; i < l('specialPopup').childNodes.length; i++) {
            if (l('specialPopup').childNodes[i].className === 'optionBox') {
                l('specialPopup').children[i].onmouseover = () => {
                    CM.Cache.CacheDragonCost();
                    Game.tooltip.dynamic = 1;
                    Game.tooltip.draw(
                        l('specialPopup'),
                        `<div style="min-width:200px;text-align:center;">${CM.Cache.CostDragonUpgrade}</div>`,
                        'this',
                    );
                    Game.tooltip.wobble();
                };
                l('specialPopup').children[i].onmouseout = () => {
...
CM.Disp.AddMenuPref = title => {
    ...
    for (const group of Object.keys(CM.ConfigGroups)) {
        const groupObject = CM.Disp.CreatePrefHeader(
            group,
            CM.ConfigGroups[group],
        ); // (group, display-name of group)
        frag.appendChild(groupObject);
        if (CM.Options.Header[group]) {
            // 0 is show, 1 is collapsed
            // Make sub-sections of Notification section
            if (group === 'Notification') {
                for (const subGroup of Object.keys(
                    CM.ConfigGroupsNotification,
                )) {
                    const subGroupObject = CM.Disp.CreatePrefHeader(
                        subGroup,
                        CM.ConfigGroupsNotification[subGroup],
                    ); // (group, display-name of group)
                    ...
                    if (CM.Options.Header[subGroup]) {
                        for (const option in CM.ConfigData) {
                            if (CM.ConfigData[option].group === subGroup) {
                                frag.appendChild(
                                    CM.Disp.CreatePrefOption(option),
                                );
                            }
                        }
                    }
                ...
                for (const option of Object.keys(CM.ConfigData)) {
                    if (CM.ConfigData[option].group === group) {
                        frag.appendChild(CM.Disp.CreatePrefOption(option));
                    }
                }
            }
        }
 ...
 */
CM.Disp.CreatePrefHeader = (config, text) => {
    const div = document.createElement('div');
    div.className = 'title';
 ...
 */
CM.Disp.CreatePrefOption = config => {
    ...
    if (CM.ConfigData[config].type === 'vol') {
        const div = document.createElement('div');
        div.className = 'listing';
    ...
    if (CM.ConfigData[config].type === 'url') {
        const div = document.createElement('div');
        div.className = 'listing';
        const span = document.createElement('span');
        ...
        a.onclick = () => {
            Game.Prompt(inputPrompt.outerHTML, [
                [
                    'Save',
                    `CM.Options['${config}'] = l(CM.ConfigPrefix + '${config}' + 'Prompt').value; CM.Config.SaveConfig(); Game.ClosePrompt(); Game.UpdateMenu();`,
                ],
                'Cancel',
            ]);
        };
    ...
    if (CM.ConfigData[config].type === 'color') {
        const div = document.createElement('div');
        for (let i = 0; i < CM.Disp.colors.length; i++) {
            ...
            const change = () => {
                CM.Options.Colors[this.targetElement.id] = this.toHEXString();
                CM.Disp.UpdateColors();
                CM.Config.SaveConfig();
                Game.UpdateMenu();
            };
    ...
    if (CM.ConfigData[config].type === 'numscale') {
        const div = document.createElement('div');
        div.className = 'listing';
        const span = document.createElement('span');
        span.className = 'option';
...
CM.Disp.UpdateColors = () => {
    let str = '';
    for (let i = 0; i < CM.Disp.colors.length; i++) {
        str += `.${CM.Disp.colorTextPre}${CM.Disp.colors[i]} { color: ${
            CM.Options.Colors[CM.Disp.colors[i]]
        }; }\n`;
    }
    for (let i = 0; i < CM.Disp.colors.length; i++) {
        str += `.${CM.Disp.colorBackPre}${
            CM.Disp.colors[i]
        } { background-color: ${CM.Options.Colors[CM.Disp.colors[i]]}; }\n`;
    }
    for (let i = 0; i < CM.Disp.colors.length; i++) {
        str += `.${CM.Disp.colorBorderPre}${
            CM.Disp.colors[i]
        } { border: 1px solid ${CM.Options.Colors[CM.Disp.colors[i]]}; }\n`;
    }
    ...
    if (Game.cpsSucked > 0) {
        ...
        if (CM.Options.Header.Wrink) {
            ...
            popAllFrag.appendChild(
                document.createTextNode(
                    `${Beautify(CM.Cache.WrinklersTotal)} / ${Beautify(
                        CM.Cache.WrinklersNormal,
                    )} `,
                ),
            );
            ...
            popFattestFrag.appendChild(
                document.createTextNode(
                    `${Beautify(CM.Cache.WrinklersFattest[0])} `,
                ),
            );
            ...
            popFattestA.onclick = () => {
                if (CM.Cache.WrinklersFattest[1]) {
                    Game.wrinklers[CM.Cache.WrinklersFattest[1]].hp = 0;
                }
            };
            ...
            if (missingHalloweenCookies.length !== 0) {
                stats.appendChild(
                    CM.Disp.CreateStatsListing(
                        ...
                        CM.Disp.CreateStatsMissDisp(missingHalloweenCookies),
                    ),
                );
            }
            if (missingChristmasCookies.length !== 0) {
                stats.appendChild(
                    CM.Disp.CreateStatsListing(
                        ...
                        CM.Disp.CreateStatsMissDisp(missingChristmasCookies),
                    ),
                );
            }
            if (missingValentineCookies.length !== 0) {
                stats.appendChild(
                    CM.Disp.CreateStatsListing(
                        ...
                        CM.Disp.CreateStatsMissDisp(missingValentineCookies),
                    ),
                );
            }
            if (missingNormalEggs.length !== 0) {
                stats.appendChild(
                    CM.Disp.CreateStatsListing(
                        ...
                        CM.Disp.CreateStatsMissDisp(missingNormalEggs),
                    ),
                );
            }
            if (missingRareEggs.length !== 0) {
                stats.appendChild(
                    CM.Disp.CreateStatsListing(
                        ...
                        CM.Disp.CreateStatsMissDisp(missingRareEggs),
                    ),
                );
            }
            if (missingPlantDrops.length !== 0) {
                stats.appendChild(
                    CM.Disp.CreateStatsListing(
                        ...
                        CM.Disp.CreateStatsMissDisp(missingPlantDrops),
                    ),
                );
            }
            ...
            if (Game.season === 'christmas') {
                stats.appendChild(
                    CM.Disp.CreateStatsListing(
                        'basic',
                        'Reindeer Reward',
                        document.createTextNode(Beautify(CM.Cache.SeaSpec)),
                    ),
                );
            }
            if (choEgg) {
                stats.appendChild(
                    CM.Disp.CreateStatsListing(
                        ...
                        document.createTextNode(Beautify(CM.Cache.lastChoEgg)),
            ...
            if (centEgg) {
                stats.appendChild(
                    CM.Disp.CreateStatsListing(
                        ...
                        document.createTextNode(
                            `${
                                Math.round((CM.Cache.CentEgg - 1) * 10000) / 100
                            }%`,
                        ),
                    ),
                );
        ...
        stats.appendChild(
            CM.Disp.CreateStatsListing(
                ...
                `Average Cookies Per Second (Past ${
                    CM.Disp.cookieTimes[CM.Options.AvgCPSHist] < 60
                        ? `${
                              CM.Disp.cookieTimes[CM.Options.AvgCPSHist]
                          } seconds`
                        : CM.Disp.cookieTimes[CM.Options.AvgCPSHist] / 60 +
                          (CM.Options.AvgCPSHist === 3 ? ' minute' : ' minutes')
                })`,
        ...
        stats.appendChild(
            CM.Disp.CreateStatsListing(
                ...
                `Average Cookie Clicks Per Second (Past ${
                    CM.Disp.clickTimes[CM.Options.AvgClicksHist]
                }${CM.Options.AvgClicksHist === 0 ? ' second' : ' seconds'})`,
                document.createTextNode(Beautify(CM.Cache.AverageClicks, 1)),
            ),
        );
            ...
            if (fortunes.length !== 0) {
                stats.appendChild(
                    CM.Disp.CreateStatsListing(
                        ...
                        CM.Disp.CreateStatsMissDisp(fortunes),
                    ),
                );
            }
        }
...
/**
 ...
 @returns	{object}		div		The header object
 */
CM.Disp.CreateStatsHeader = (text, config) => {
    const div = document.createElement('div');
    div.className = 'listing';
...
/**
 ...
 @returns	{object}		div			The option object
 */
CM.Disp.CreateStatsListing = (type, name, text, placeholder) => {
    const div = document.createElement('div');
    div.className = 'listing';
    ...
    if (type === 'withTooltip') {
        ...
        tooltip.onmouseover = () => {
            Game.tooltip.draw(this, escape(CM.Disp[placeholder].innerHTML));
        };
 ...
 */
CM.Disp.CreateStatsMissDisp = theMissDisp => {
    const frag = document.createDocumentFragment();
 ...
 */
CM.Disp.CreateStatsLuckySection = () => {
    // This sets which tooltip to display for certain stats
    const goldCookTooltip = CM.Sim.auraMult("Dragon's Fortune")
    ...
    const luckyColor =
        Game.cookies + CM.Disp.GetWrinkConfigBank() < CM.Cache.Lucky
            ? CM.Disp.colorRed
            : CM.Disp.colorGreen;
    const luckyTime =
        Game.cookies + CM.Disp.GetWrinkConfigBank() < CM.Cache.Lucky
    ...
    const luckyColorFrenzy =
        Game.cookies + CM.Disp.GetWrinkConfigBank() < CM.Cache.LuckyFrenzy
            ? CM.Disp.colorRed
            : CM.Disp.colorGreen;
    const luckyTimeFrenzy =
        Game.cookies + CM.Disp.GetWrinkConfigBank() < CM.Cache.LuckyFrenzy
            ? CM.Disp.FormatTime(
                  (CM.Cache.LuckyFrenzy -
                      (Game.cookies + CM.Disp.GetWrinkConfigBank())) /
                      CM.Disp.GetCPS(),
              )
            : '';
    ...
    luckyRewardMaxSpan.textContent =
        Beautify(CM.Cache.LuckyReward) +
        (luckySplit ? ` / ${Beautify(CM.Cache.LuckyWrathReward)}` : '');
    section.appendChild(
        CM.Disp.CreateStatsListing(
            'withTooltip',
    ...
    luckyRewardFrenzyMaxSpan.className =
        CM.Disp.colorTextPre + luckyRewardFrenzyMaxSpan;
    luckyRewardFrenzyMaxSpan.textContent =
        Beautify(CM.Cache.LuckyRewardFrenzy) +
        (luckySplit ? ` / ${Beautify(CM.Cache.LuckyWrathRewardFrenzy)}` : '');
    section.appendChild(
        CM.Disp.CreateStatsListing(
            'withTooltip',
            `"Lucky!" Reward (MAX) (Frenzy)${
                luckySplit ? ' (Golden / Wrath)' : ''
            }`,
    ...
    const luckyCurBase =
        Math.min(
            (Game.cookies + CM.Disp.GetWrinkConfigBank()) * 0.15,
            CM.Cache.NoGoldSwitchCookiesPS *
                CM.Cache.DragonsFortuneMultAdjustment *
                60 *
                15,
        ) + 13;
    ...
    luckyCurSpan.textContent =
        Beautify(CM.Cache.GoldenCookiesMult * luckyCurBase) +
        (luckySplit
            ? ` / ${Beautify(CM.Cache.WrathCookiesMult * luckyCurBase)}`
            : '');
    section.appendChild(
        CM.Disp.CreateStatsListing(
            'withTooltip',
 ...
 */
CM.Disp.CreateStatsChainSection = () => {
    // This sets which tooltip to display for certain stats
    const goldCookTooltip = CM.Sim.auraMult("Dragon's Fortune")
        ? 'GoldCookDragonsFortuneTooltipPlaceholder'
        : 'GoldCookTooltipPlaceholder';
    ...
    const chainColor =
        Game.cookies + CM.Disp.GetWrinkConfigBank() < CM.Cache.Chain
            ? CM.Disp.colorRed
            : CM.Disp.colorGreen;
    const chainTime =
        Game.cookies + CM.Disp.GetWrinkConfigBank() < CM.Cache.Chain
    ...
    const chainWrathColor =
        Game.cookies + CM.Disp.GetWrinkConfigBank() < CM.Cache.ChainWrath
            ? CM.Disp.colorRed
            : CM.Disp.colorGreen;
    ...
    const chainColorFrenzy =
        Game.cookies + CM.Disp.GetWrinkConfigBank() < CM.Cache.ChainFrenzy
            ? CM.Disp.colorRed
            : CM.Disp.colorGreen;
    ...
    const chainWrathColorFrenzy =
        Game.cookies + CM.Disp.GetWrinkConfigBank() < CM.Cache.ChainFrenzyWrath
            ? CM.Disp.colorRed
            : CM.Disp.colorGreen;
    ...
    chainWrathReqFrenSpan.className =
        CM.Disp.colorTextPre + chainWrathColorFrenzy;
    ...
    section.appendChild(
        CM.Disp.CreateStatsListing(
            ...
            document.createTextNode(
                `${Beautify(CM.Cache.ChainReward)} / ${Beautify(
                    CM.Cache.ChainWrathReward,
                )}`,
            ),
            goldCookTooltip,
        ),
    ...
    section.appendChild(
        CM.Disp.CreateStatsListing(
            ...
            document.createTextNode(
                `${Beautify(CM.Cache.ChainFrenzyReward)} / ${Beautify(
    ...
    const chainCurMax = Math.min(
        CM.Cache.NoGoldSwitchCookiesPS *
            ...
            60 *
            6,
        (Game.cookies + CM.Disp.GetWrinkConfigBank()) * 0.5,
    );
    const chainCur = CM.Cache.MaxChainMoni(
        7,
        chainCurMax,
        CM.Cache.GoldenCookiesMult,
    );
    const chainCurWrath = CM.Cache.MaxChainMoni(
        6,
        chainCurMax,
        CM.Cache.WrathCookiesMult,
    );
    section.appendChild(
        CM.Disp.CreateStatsListing(
            'withTooltip',
 ...
 */
CM.Disp.CreateStatsSpellsSection = () => {
    const section = document.createElement('div');
    section.className = 'CMStatsSpellsSection';

    const conjureColor =
        Game.cookies + CM.Disp.GetWrinkConfigBank() < CM.Cache.Conjure
            ? CM.Disp.colorRed
            : CM.Disp.colorGreen;
    const conjureTime =
        Game.cookies + CM.Disp.GetWrinkConfigBank() < CM.Cache.Conjure
    ...
    section.appendChild(
        CM.Disp.CreateStatsListing(
            ...
            document.createTextNode(CM.Disp.Beautify(CM.Cache.ConjureReward)),
            'GoldCookTooltipPlaceholder',
        ),
    );

    const conjureFrenzyColor =
        Game.cookies + CM.Disp.GetWrinkConfigBank() < CM.Cache.Conjure * 7
            ? CM.Disp.colorRed
            : CM.Disp.colorGreen;
    ...
    section.appendChild(
        CM.Disp.CreateStatsListing(
            ...
            document.createTextNode(
                CM.Disp.Beautify(CM.Cache.ConjureReward * 7),
            ),
            'GoldCookTooltipPlaceholder',
        ),
    );
    section.appendChild(
        CM.Disp.CreateStatsListing(
            ...
            document.createTextNode(CM.Disp.Beautify(conjureFrenzyCur)),
            'GoldCookTooltipPlaceholder',
        ),
    );
    if (CM.Cache.Edifice) {
        section.appendChild(
            CM.Disp.CreateStatsListing(
                ...
                document.createTextNode(
                    `${CM.Disp.Beautify(CM.Cache.Edifice)} (${
                        CM.Cache.EdificeBuilding
                    })`,
                ),
                'GoldCookTooltipPlaceholder',
            ),
...
CM.Disp.CreateStatsPrestigeSection = () => {
    ...
    cookiesNextSmall.textContent = ` (${CM.Disp.FormatTime(
        neededCook / CM.Cache.AvgCPSWithChoEgg,
        1,
    )})`;
...
CM.Disp.AddMissingUpgrades = () => {
    if (CM.Cache.MissingUpgradesPrestige) {
        const prestigeUpgradesOwned =
            ...
            l('menu').children[5].children[3].children.length;
        const title = document.createElement('div');
        title.id = 'CMMissingUpgradesPrestigeTitle';
        title.className = 'listing';
        const titlefrag = document.createElement('div');
        titlefrag.innerHTML = `<b>Missing Prestige upgrades:</b> ${prestigeUpgradesOwned}/${
            Game.PrestigeUpgrades.length
        } (${Math.floor(
            (prestigeUpgradesOwned / Game.PrestigeUpgrades.length) * 100,
        )}%)`;
    ...
    if (CM.Cache.MissingUpgrades) {
        ...
        if (Game.UpgradesOwned) {
            normalUpgradesOwned =
                ...
                l('menu').children[6].childNodes[2].children.length;
        }
        const title = document.createElement('div');
        title.id = 'CMMissingUpgradesTitle';
        title.className = 'listing';
        const titlefrag = document.createElement('div');
        titlefrag.innerHTML = `<b>Missing normal upgrades:</b> ${normalUpgradesOwned}/${
        ...
        title.appendChild(titlefrag);
        l('menu').children[6].insertBefore(
            title,
            l('menu').children[6].childNodes[3],
        );
        const upgrades = document.createElement('div');
        upgrades.className = 'listing crateBox';
    ...
    if (CM.Cache.MissingUpgradesCookies) {
        const cookieUpgradesOwned =
            Game.UpgradesByPool.cookie.length -
            l('menu').children[6].lastChild.children.length;
        const title = document.createElement('div');
        title.id = 'CMMissingUpgradesCookiesTitle';
        title.className = 'listing';
        const titlefrag = document.createElement('div');
        titlefrag.innerHTML = `<b>Missing Cookie upgrades:</b> ${cookieUpgradesOwned}/${
            Game.UpgradesByPool.cookie.length
        } (${Math.floor(
            (cookieUpgradesOwned / Game.UpgradesByPool.cookie.length) * 100,
        )}%)`;
        title.appendChild(titlefrag);
...
CM.Disp.TooltipText = [
    [
        'GoldCookTooltipPlaceholder',
        'Calculated with Golden Switch off',
        '200px',
    ],
    [
        'GoldCookDragonsFortuneTooltipPlaceholder',
 ...
 */
CM.Disp.buffColors = {
    Frenzy: CM.Disp.colorYellow,
    'Dragon Harvest': CM.Disp.colorBrown,
    'Elder frenzy': CM.Disp.colorGreen,
    Clot: CM.Disp.colorRed,
    'Click frenzy': CM.Disp.colorBlue,
    Dragonflight: CM.Disp.colorPink,
};
CM.Disp.GCTimers = {};
...
CM.ReplaceNative = () => {
    ...
    CM.Backup.CalculateGainsMod = new Function(
        `return ${Game.CalculateGains.toString()
            .split("ages');")
            .join("ages');CM.Sim.DateAges = Date.now();")
            .split("if (Game.Has('Century")
    ...
    CM.Backup.tooltip.drawMod = new Function(
        `return ${Game.tooltip.draw
            .toString()
            .split('this')
            .join('Game.tooltip')}`,
    )();
    Game.tooltip.draw = (from, text, origin) => {
        CM.Backup.tooltip.drawMod(from, text, origin);
    };

    CM.Backup.tooltip.update = Game.tooltip.update;
    CM.Backup.tooltip.updateMod = new Function(
    ...
    l('bigCookie').addEventListener(
        'click',
        () => {
            CM.Main.FixMouseY(Game.ClickCookie);
        },
        false,
    );
  ...
  */
    ...
    CM.Disp.sayTime = (time, detail) => {
        if (Number.isNaN(time) || time <= 0) {
            return CM.Backup.sayTime(time, detail);
        }
        return CM.Disp.FormatTime(time / Game.fps, 1);
    };
...
CM.ReplaceNativeGrimoireDraw = () => {
    if (
        !CM.HasReplaceNativeGrimoireDraw &&
        Game.Objects['Wizard tower'].minigameLoaded
    ) {
        const minigame = Game.Objects['Wizard tower'].minigame;
        CM.Backup.GrimoireDraw = minigame.draw;
        Game.Objects['Wizard tower'].minigame.draw = () => {
            CM.Backup.GrimoireDraw();
            if (
                CM.Options.GrimoireBar === 1 &&
                minigame.magic < minigame.magicM
            ) {
                minigame.magicBarTextL.innerHTML += ` (${CM.Disp.FormatTime(
                    CM.Disp.CalculateGrimoireRefillTime(
                        minigame.magic,
                        minigame.magicM,
                        minigame.magicM,
...
CM.DelayInit = () => {
    ...
    for (const i of Object.keys(CM.Disp.TooltipText)) {
        CM.Disp.CreateSimpleTooltip(
            CM.Disp.TooltipText[i][0],
            CM.Disp.TooltipText[i][1],
            CM.Disp.TooltipText[i][2],
        );
    }
    CM.Disp.CreateWrinklerButtons();
    ...
    if (Game.prefs.popups) {
        Game.Popup(
            `Cookie Monster version ${CM.VersionMajor}.${CM.VersionMinor} loaded!`,
        );
    } else {
        Game.Notify(
            `Cookie Monster version ${CM.VersionMajor}.${CM.VersionMinor} loaded!`,
    ...
    if (Game.Objects['Wizard tower'].minigameLoaded) {
        ...
        for (const i in Game.Objects['Wizard tower'].minigame.spellsById) {
            if (l(`grimoireSpell${i}`).onmouseover != null) {
                CM.Main.TooltipGrimoireBackup[i] = l(
                    `grimoireSpell${i}`,
                ).onmouseover;
                l(`grimoireSpell${i}`).onmouseover = () => {
                    Game.tooltip.dynamic = 1;
                    Game.tooltip.draw(
                        this,
                        () => CM.Disp.Tooltip('g', `${i}`),
...
CM.Main.ReplaceTooltipLump = () => {
    if (Game.canLumps()) {
        ...
        l('lumps').onmouseover = () => {
            Game.tooltip.dynamic = 1;
            Game.tooltip.draw(this, () => CM.Disp.Tooltip('s', 'Lump'), 'this');
            Game.tooltip.wobble();
        };
    }
};
...
CM.Main.ReplaceTooltipGarden = () => {
    if (Game.Objects.Farm.minigameLoaded) {
        l('gardenTool-1').onmouseover = () => {
            Game.tooltip.dynamic = 1;
            Game.tooltip.draw(
                this,
                () => {
                    return CM.Disp.Tooltip('ha', 'HarvestAllButton');
                },
                'this',
            );
            Game.tooltip.wobble();
...
CM.Main.CheckGoldenCookie = () => {
    ...
    if (CM.Main.lastGoldenCookieState !== Game.shimmerTypes.golden.n) {
        ...
        if (CM.Main.lastGoldenCookieState) {
            ...
            for (const i of Object.keys(Game.shimmers)) {
                if (
                    typeof CM.Disp.GCTimers[Game.shimmers[i].id] === 'undefined'
                ) {
        ...
        CM.Main.lastSpawnedGoldenCookieState =
            CM.Main.currSpawnedGoldenCookieState;
        if (CM.Main.currSpawnedGoldenCookieState === 0) {
            CM.Cache.spawnedGoldenShimmer = 0;
        }
    } else if (CM.Options.GCTimer === 1 && CM.Main.lastGoldenCookieState) {
        for (const i of Object.keys(CM.Disp.GCTimers)) {
            CM.Disp.GCTimers[i].style.opacity =
                CM.Cache.goldenShimmersByID[i].l.style.opacity;
            CM.Disp.GCTimers[i].style.transform =
                CM.Cache.goldenShimmersByID[i].l.style.transform;
            CM.Disp.GCTimers[i].textContent = Math.ceil(
                CM.Cache.goldenShimmersByID[i].life / Game.fps,
            );
        }
    }
};
...
CM.Main.CheckSeasonPopup = () => {
    if (CM.Main.lastSeasonPopupState !== Game.shimmerTypes.reindeer.spawned) {
        ...
        for (const i of Object.keys(Game.shimmers)) {
            if (
                Game.shimmers[i].spawnLead &&
                Game.shimmers[i].type === 'reindeer'
            ) {
...
CM.Main.CheckTickerFortune = () => {
    if (
        CM.Main.lastTickerFortuneState !==
...
CM.Main.CheckGardenTick = () => {
    if (
        ...
        if (
            ...
            CM.Main.lastGardenNextStep < Date.now()
        ) {
            CM.Disp.Flash(3, 'GardFlash');
            CM.Disp.PlaySound(
                CM.Options.GardSoundURL,
                'GardSound',
                'GardVolume',
            );
        }
...
CM.Main.CheckWrinklerCount = () => {
    if (Game.elderWrath > 0) {
        ...
        if (CurrentWrinklers > CM.Main.lastWrinklerCount) {
            ...
            if (
                CurrentWrinklers === Game.getWrinklersMax() &&
                CM.Options.WrinklerMaxSound
            ) {
                CM.Disp.PlaySound(
                    CM.Options.WrinklerMaxSoundURL,
                    'WrinklerMaxSound',
                    'WrinklerMaxVolume',
                );
            } else {
                CM.Disp.PlaySound(
                    CM.Options.WrinklerSoundURL,
                    'WrinklerSound',
                    'WrinklerVolume',
                );
            }
 ...
 */
CM.Main.AddWrinklerAreaDetect = () => {
    l('backgroundLeftCanvas').onmouseover = () => {
        CM.Disp.TooltipWrinklerArea = 1;
    };
    l('backgroundLeftCanvas').onmouseout = () => {
        CM.Disp.TooltipWrinklerArea = 0;
        Game.tooltip.hide();
        for (const i of Object.keys(Game.wrinklers)) {
            CM.Disp.TooltipWrinklerBeingShown[i] = 0;
 ...
 */
CM.Main.FixMouseY = target => {
    if (CM.Options.TimerBar === 1 && CM.Options.TimerBarPos === 0) {
        const timerBarHeight = parseInt(CM.Disp.TimerBar.style.height, 10);
...
CM.Sim.BuildingGetPrice = (build, basePrice, start, free, increase) => {
    // let price = 0;
    // for (let i = Math.max(0, start); i < Math.max(0, start + increase); i++) {
    //  price += basePrice * Game.priceIncrease ** Math.max(0, i - free);
    // }
...
CM.Sim.BuildingSell = (build, basePrice, start, free, amount, noSim) => {
    // let price = 0;
    ...
    for (let i = 0; i < amount; i++) {
        ...
        price = noSim
            ...
            : CM.Sim.modifyBuildingPrice(build, price);
        price = Math.ceil(price);
        const giveBack = noSim
            ? build.getSellMultiplier()
            : CM.Sim.getSellMultiplier();
    ...
    if (CM.Sim.Achievements[what]) {
        if (CM.Sim.Achievements[what].won === 0) {
            ...
            if (Game.Achievements[what].pool !== 'shadow') {
                CM.Sim.AchievementsOwned += 1;
            }
        }
    }
};

CM.Sim.HasAchiev = new Function(
    `return ${Game.HasAchiev.toString().split('Game').join('CM.Sim')}`,
)();

CM.Sim.GetHeavenlyMultiplier = new Function(
    `return ${Game.GetHeavenlyMultiplier.toString()
        .split('Game.Has')
        .join('CM.Sim.Has')
        .split('Game.hasAura')
        .join('CM.Sim.hasAura')
...
CM.Sim.hasAura = what => {
    if (
        Game.dragonAuras[CM.Sim.dragonAura].name === what ||
        Game.dragonAuras[CM.Sim.dragonAura2].name === what
    ) {
        return true;
    }
    return false;
...
CM.Sim.auraMult = what => {
    ...
    if (
        Game.dragonAuras[CM.Sim.dragonAura].name === 'Reality Bending' ||
        Game.dragonAuras[CM.Sim.dragonAura2].name === 'Reality Bending'
    ) {
        n += 0.1;
    }
    return n;
...
CM.Sim.hasGod = what => {
    ...
    const possibleGods = CM.Sim.Objects.Temple.minigame.gods;
    const god = possibleGods[what];
    for (let i = 0; i < 3; i++) {
...
CM.Sim.GetTieredCpsMult = new Function(
    `return ${Game.GetTieredCpsMult.toString()
        .split('Game.Has')
        .join('CM.Sim.Has')
        .split('me.tieredUpgrades')
        .join('Game.Objects[me.name].tieredUpgrades')
        .split('me.synergies')
        .join('Game.Objects[me.name].synergies')
        ...
        .split('me.grandma')
        .join('Game.Objects[me.name].grandma')
        .split('me.id')
        .join('Game.Objects[me.name].id')
        .split("Game.Objects['Grandma']")
        .join("CM.Sim.Objects['Grandma']")
        .split('me.fortune')
        .join('Game.Objects[me.name].fortune')}`,
)();
 ...
 */
CM.Sim.InitialBuildingData = buildingName => {
 ...
 */
CM.Sim.InitUpgrade = upgradeName => {
...
/**
 ...
 so this function just returns an empty object.
 */
CM.Sim.InitAchievement = achievementName => {
...
CM.Sim.CopyData = () => {
    ...
    for (const i of Object.keys(Game.Objects)) {
        ...
        if (you === undefined) {
            // New building!
            you = CM.Sim.InitialBuildingData(i);
            CM.Sim.Objects[i] = CM.Sim.InitialBuildingData(i);
...
CM.Sim.CalculateGains = () => {
    ...
    for (const i of Object.keys(Game.Objects)) {
        ...
        if (
            CM.Sim.Objects[i].minigameLoaded &&
            CM.Sim.Objects[i].minigame.effs
        ) {
            const myEffs = CM.Sim.Objects[i].minigame.effs;
    ...
    if (Game.ascensionMode !== 1) {
        mult +=
            parseFloat(CM.Sim.prestige) *
            0.01 *
            CM.Sim.heavenlyPower *
            CM.Sim.GetHeavenlyMultiplier();
    }

    mult *= CM.Sim.eff('cps');

    if (CM.Sim.Has('Heralds') && Game.ascensionMode !== 1) {
    ...
    for (const i of Object.keys(Game.cookieUpgrades)) {
        ...
        if (CM.Sim.Has(me.name)) {
            mult *=
                1 +
                (typeof me.power === 'function' ? me.power(me) : me.power) *
                    0.01;
        }
    }

    if (CM.Sim.Has('Specialized chocolate chips')) {
        mult *= 1.01;
    }
    if (CM.Sim.Has('Designer cocoa beans')) {
        mult *= 1.02;
    }
    if (CM.Sim.Has('Underworld ovens')) {
        mult *= 1.03;
    }
    if (CM.Sim.Has('Exotic nuts')) {
        mult *= 1.04;
    }
    if (CM.Sim.Has('Arcane sugar')) {
        mult *= 1.05;
    }

    if (CM.Sim.Has('Increased merriness')) {
        mult *= 1.15;
    }
    if (CM.Sim.Has('Improved jolliness')) {
        mult *= 1.15;
    }
    if (CM.Sim.Has('A lump of coal')) {
        mult *= 1.01;
    }
    if (CM.Sim.Has('An itchy sweater')) {
        mult *= 1.01;
    }
    if (CM.Sim.Has("Santa's dominion")) {
        mult *= 1.2;
    }

    if (CM.Sim.Has('Fortune #100')) {
        mult *= 1.01;
    }
    if (CM.Sim.Has('Fortune #101')) {
        mult *= 1.07;
    }

    if (CM.Sim.Has('Dragon scale')) {
        mult *= 1.03;
    }
    ...
    if (CM.Sim.Objects.Temple.minigameLoaded) {
        ...
        if (godLvl === 1) {
            mult *=
                ...
                0.15 *
                    Math.sin(
                        (CM.Sim.DateAges / 1000 / (60 * 60 * 3)) * Math.PI * 2,
                    );
        } else if (godLvl === 2) {
            mult *=
                1 +
                0.15 *
                    Math.sin(
                        (CM.Sim.DateAges / 1000 / (60 * 60 * 12)) * Math.PI * 2,
                    );
        } else if (godLvl === 3) {
            mult *=
                1 +
                0.15 *
                    Math.sin(
                        (CM.Sim.DateAges / 1000 / (60 * 60 * 24)) * Math.PI * 2,
                    );
        }
    ...
    if (CM.Sim.Has("Santa's legacy")) {
        mult *= 1 + (Game.santaLevel + 1) * 0.03;
    }

    const milkProgress = CM.Sim.AchievementsOwned / 25;
    let milkMult = 1;
    if (CM.Sim.Has("Santa's milk and cookies")) {
        milkMult *= 1.05;
    }
    // if (CM.Sim.hasAura('Breath of Milk')) {
    //     milkMult *= 1.05;
    // }
    ...
    if (CM.Sim.Has('Kitten helpers')) {
        catMult *= 1 + milkProgress * 0.1 * milkMult;
    }
    if (CM.Sim.Has('Kitten workers')) {
        catMult *= 1 + milkProgress * 0.125 * milkMult;
    }
    if (CM.Sim.Has('Kitten engineers')) {
        catMult *= 1 + milkProgress * 0.15 * milkMult;
    }
    if (CM.Sim.Has('Kitten overseers')) {
        catMult *= 1 + milkProgress * 0.175 * milkMult;
    }
    if (CM.Sim.Has('Kitten managers')) {
        catMult *= 1 + milkProgress * 0.2 * milkMult;
    }
    if (CM.Sim.Has('Kitten accountants')) {
        catMult *= 1 + milkProgress * 0.2 * milkMult;
    }
    if (CM.Sim.Has('Kitten specialists')) {
        catMult *= 1 + milkProgress * 0.2 * milkMult;
    }
    if (CM.Sim.Has('Kitten experts')) {
        catMult *= 1 + milkProgress * 0.2 * milkMult;
    }
    if (CM.Sim.Has('Kitten consultants')) {
        catMult *= 1 + milkProgress * 0.2 * milkMult;
    }
    if (CM.Sim.Has('Kitten assistants to the regional manager')) {
        catMult *= 1 + milkProgress * 0.175 * milkMult;
    }
    if (CM.Sim.Has('Kitten marketeers')) {
        catMult *= 1 + milkProgress * 0.15 * milkMult;
    }
    if (CM.Sim.Has('Kitten analysts')) {
        catMult *= 1 + milkProgress * 0.125 * milkMult;
    }
    if (CM.Sim.Has('Kitten executives')) {
        catMult *= 1 + milkProgress * 0.115 * milkMult;
    }
    if (CM.Sim.Has('Kitten angels')) {
        catMult *= 1 + milkProgress * 0.1 * milkMult;
    }
    if (CM.Sim.Has('Fortune #103')) {
        catMult *= 1 + milkProgress * 0.05 * milkMult;
    }

    for (const i of Object.keys(CM.Sim.Objects)) {
        const me = CM.Sim.Objects[i];
        let storedCps = typeof me.cps === 'function' ? me.cps(me) : me.cps;
        if (Game.ascensionMode !== 1) {
            storedCps *= (1 + me.level * 0.01) * buildMult;
        }
        if (
            me.name === 'Grandma' &&
            CM.Sim.Has('Milkhelp&reg; lactose intolerance relief tablets')
        ) {
    ...
    let eggMult = 1;
    if (CM.Sim.Has('Chicken egg')) {
        eggMult *= 1.01;
    }
    if (CM.Sim.Has('Duck egg')) {
        eggMult *= 1.01;
    }
    if (CM.Sim.Has('Turkey egg')) {
        eggMult *= 1.01;
    }
    if (CM.Sim.Has('Quail egg')) {
        eggMult *= 1.01;
    }
    if (CM.Sim.Has('Robin egg')) {
        eggMult *= 1.01;
    }
    if (CM.Sim.Has('Ostrich egg')) {
        eggMult *= 1.01;
    }
    if (CM.Sim.Has('Cassowary egg')) {
        eggMult *= 1.01;
    }
    if (CM.Sim.Has('Salmon roe')) {
        eggMult *= 1.01;
    }
    if (CM.Sim.Has('Frogspawn')) {
        eggMult *= 1.01;
    }
    if (CM.Sim.Has('Shark egg')) {
        eggMult *= 1.01;
    }
    if (CM.Sim.Has('Turtle egg')) {
        eggMult *= 1.01;
    }
    if (CM.Sim.Has('Ant larva')) {
        eggMult *= 1.01;
    }
    if (CM.Sim.Has('Century egg')) {
        // The boost increases a little every day, with diminishing returns up to +10% on the 100th day
        let day =
            (Math.floor((CM.Sim.DateCentury - Game.startDate) / 1000 / 10) *
                10) /
            60 /
            60 /
            24;
    ...
    if (CM.Sim.Has('Sugar baking')) {
        mult *= 1 + Math.min(100, Game.lumps) * 0.01;
    }

    // if (CM.Sim.hasAura('Radiant Appetite')) {
    //     mult *= 2;
    // }
    ...
    for (const i of Object.keys(Game.CpsAchievements)) {
        if (rawCookiesPs >= Game.CpsAchievements[i].threshold) {
            CM.Sim.Win(Game.CpsAchievements[i].name);
        }
    }
    ...
    if (CM.Sim.Has('Shimmering veil [off]')) {
        let veilMult = 0.5;
        if (CM.Sim.Has('Reinforced membrane')) {
            veilMult += 0.1;
        }
        mult *= 1 + veilMult;
    }
...
CM.Sim.CheckOtherAchiev = () => {
    ...
    for (const i of Object.keys(Game.GrandmaSynergies)) {
        if (CM.Sim.Has(Game.GrandmaSynergies[i])) {
            grandmas += 1;
        }
    }
    if (!CM.Sim.HasAchiev('Elder') && grandmas >= 7) {
        CM.Sim.Win('Elder');
    }
    if (!CM.Sim.HasAchiev('Veteran') && grandmas >= 14) {
        CM.Sim.Win('Veteran');
    }
    ...
    for (const i of Object.keys(CM.Sim.Objects)) {
        ...
        if (!CM.Sim.HasAchiev('Mathematician')) {
            if (
                CM.Sim.Objects[i].amount <
                Math.min(
                    128,
                    2 ** Game.ObjectsById.length - Game.Objects[i].id - 1,
                )
            ) {
                mathematician = 0;
            }
    ...
    if (minAmount >= 1) {
        CM.Sim.Win('One with everything');
    }
    if (mathematician === 1) {
        CM.Sim.Win('Mathematician');
    }
    if (base10 === 1) {
        CM.Sim.Win('Base 10');
    }
    if (minAmount >= 100) {
        CM.Sim.Win('Centennial');
    }
    if (minAmount >= 150) {
        CM.Sim.Win('Centennial and a half');
    }
    if (minAmount >= 200) {
        CM.Sim.Win('Bicentennial');
    }
    if (minAmount >= 250) {
        CM.Sim.Win('Bicentennial and a half');
    }
    if (minAmount >= 300) {
        CM.Sim.Win('Tricentennial');
    }
    if (minAmount >= 350) {
        CM.Sim.Win('Tricentennial and a half');
    }
    if (minAmount >= 400) {
        CM.Sim.Win('Quadricentennial');
    }
    if (minAmount >= 450) {
        CM.Sim.Win('Quadricentennial and a half');
    }
    if (minAmount >= 500) {
        CM.Sim.Win('Quincentennial');
    }
    if (minAmount >= 550) {
        CM.Sim.Win('Quincentennial and a half');
    }
    if (minAmount >= 600) {
        CM.Sim.Win('Sexcentennial');
    }
    ...
    if (buildingsOwned >= 2000) {
        CM.Sim.Win('Lord of Constructs');
    }
    if (buildingsOwned >= 4000) {
        CM.Sim.Win('Grand design');
    }
    if (buildingsOwned >= 8000) {
        CM.Sim.Win('Ecumenopolis');
    }

    if (CM.Sim.UpgradesOwned >= 20) {
        CM.Sim.Win('Enhancer');
    }
    if (CM.Sim.UpgradesOwned >= 50) {
        CM.Sim.Win('Augmenter');
    }
    if (CM.Sim.UpgradesOwned >= 100) {
        CM.Sim.Win('Upgrader');
    }
    if (CM.Sim.UpgradesOwned >= 200) {
        CM.Sim.Win('Lord of Progress');
    }
    if (CM.Sim.UpgradesOwned >= 300) {
        CM.Sim.Win('The full picture');
    }
    if (CM.Sim.UpgradesOwned >= 400) {
        CM.Sim.Win("When there's nothing left to add");
    }

    if (buildingsOwned >= 4000 && CM.Sim.UpgradesOwned >= 300) {
        CM.Sim.Win('Polymath');
    }
    if (buildingsOwned >= 8000 && CM.Sim.UpgradesOwned >= 400) {
        CM.Sim.Win('Renaissance baker');
    }

    if (CM.Sim.Objects.Cursor.amount + CM.Sim.Objects.Grandma.amount >= 777) {
        CM.Sim.Win('The elder scrolls');
    }

    let hasAllHalloCook = true;
    for (const i of Object.keys(CM.Data.HalloCookies)) {
        if (!CM.Sim.Has(CM.Data.HalloCookies[i])) {
            hasAllHalloCook = false;
        }
    }
    if (hasAllHalloCook) {
        CM.Sim.Win('Spooky cookies');
    }

    let hasAllChristCook = true;
    for (const i of Object.keys(CM.Data.ChristCookies)) {
        if (!CM.Sim.Has(CM.Data.ChristCookies[i])) {
            hasAllChristCook = false;
        }
    }
    if (hasAllChristCook) {
        CM.Sim.Win('Let it snow');
    }
    ...
    if (CM.Sim.Has('Fortune cookies')) {
        ...
        let fortunes = 0;
        for (const i of Object.keys(list)) {
            if (CM.Sim.Has(list[i].name)) {
                fortunes += 1;
            }
        }
        if (fortunes >= list.length) {
            CM.Sim.Win('O Fortuna');
        }
    }
};

CM.Sim.BuyBuildings = (amount, target) => {
    CM.Cache[target] = [];
    for (const i of Object.keys(Game.Objects)) {
        CM.Sim.CopyData();
        const me = CM.Sim.Objects[i];
        ...
        if (i === 'Cursor') {
            ...
            if (me.amount >= 2) {
                CM.Sim.Win('Double-click');
            }
            if (me.amount >= 50) {
                CM.Sim.Win('Mouse wheel');
            }
            if (me.amount >= 100) {
                CM.Sim.Win('Of Mice and Men');
            }
            if (me.amount >= 200) {
                CM.Sim.Win('The Digital');
            }
            if (me.amount >= 300) {
                CM.Sim.Win('Extreme polydactyly');
            }
            if (me.amount >= 400) {
                CM.Sim.Win('Dr. T');
            }
            if (me.amount >= 500) {
                CM.Sim.Win('Thumbs, phalanges, metacarpals');
            }
            if (me.amount >= 600) {
                CM.Sim.Win('With her finger and her thumb');
            }
            if (me.amount >= 700) {
                CM.Sim.Win('Gotta hand it to you');
            }
            if (me.amount >= 800) {
                CM.Sim.Win("The devil's workshop");
            }
        } else {
            for (const j in Game.Objects[me.name].tieredAchievs) {
                if (
                    me.amount >=
                    Game.Tiers[Game.Objects[me.name].tieredAchievs[j].tier]
                        .achievUnlock
                ) {
...
CM.Sim.BuyUpgrades = () => {
    ...
    for (const i of Object.keys(Game.Upgrades)) {
        if (
            ...
            (Game.Upgrades[i].bought === 0 &&
                Game.Upgrades[i].unlocked &&
                Game.Upgrades[i].pool !== 'prestige')
        ) {
            CM.Sim.CopyData();
            const me = CM.Sim.Upgrades[i];
            me.bought = 1;
            if (Game.CountsAsUpgradeOwned(Game.Upgrades[i].pool)) {
                CM.Sim.UpgradesOwned += 1;
            }

            if (i === 'Elder Pledge') {
                CM.Sim.pledges += 1;
                if (CM.Sim.pledges > 0) {
                    CM.Sim.Win('Elder nap');
                }
                if (CM.Sim.pledges >= 5) {
                    CM.Sim.Win('Elder slumber');
                }
            ...
            if (diffMouseCPS) {
                CM.Cache.Upgrades[i].bonusMouse = diffMouseCPS;
            }
        }
    }
};
 ...
 */
CM.Sim.CalculateChangeAura = aura => {
    CM.Sim.CopyData();
    ...
    if (
        ...
        for (let i = Game.ObjectsById.length; i > -1; --i) {
            if (Game.ObjectsById[i].amount > 0) {
                const highestBuilding =
                    CM.Sim.Objects[Game.ObjectsById[i].name].name;
                CM.Sim.Objects[highestBuilding].amount -= 1;
                CM.Sim.buildingsOwned -= 1;
                price =
                    CM.Sim.Objects[highestBuilding].basePrice *
                    Game.priceIncrease **
                        Math.max(
                            0,
                            CM.Sim.Objects[highestBuilding].amount -
                                1 -
                                CM.Sim.Objects[highestBuilding].free,
                        );
                price = Game.modifyBuildingPrice(
                    CM.Sim.Objects[highestBuilding],
                    price,
                );
                price = Math.ceil(price);
                break;
...
CM.Sim.NoGoldSwitchCookiesPS = () => {
    if (Game.Has('Golden switch [off]')) {
        ...
        CM.Cache.NoGoldSwitchCookiesPS = Game.cookiesPs;
    }
};

CM.Sim.ResetBonus = possiblePresMax => {
    let lastAchievementsOwned = -1;
    ...
    if (CM.Cache.RealCookiesEarned >= 1000000) {
        CM.Sim.Win('Sacrifice');
    }
    if (CM.Cache.RealCookiesEarned >= 1000000000) {
        CM.Sim.Win('Oblivion');
    }
    if (CM.Cache.RealCookiesEarned >= 1000000000000) {
        CM.Sim.Win('From scratch');
    }
    if (CM.Cache.RealCookiesEarned >= 1000000000000000) {
        CM.Sim.Win('Nihilism');
    }
    if (CM.Cache.RealCookiesEarned >= 1000000000000000000) {
        CM.Sim.Win('Dematerialize');
    }
    if (CM.Cache.RealCookiesEarned >= 1000000000000000000000) {
        CM.Sim.Win('Nil zero zilch');
    }
    if (CM.Cache.RealCookiesEarned >= 1000000000000000000000000) {
        CM.Sim.Win('Transcendence');
    }
    if (CM.Cache.RealCookiesEarned >= 1000000000000000000000000000) {
        CM.Sim.Win('Obliterate');
    }
    if (CM.Cache.RealCookiesEarned >= 1000000000000000000000000000000) {
        CM.Sim.Win('Negative void');
    }
    if (CM.Cache.RealCookiesEarned >= 1000000000000000000000000000000000) {
        CM.Sim.Win('To crumbs, you say?');
    }
    if (CM.Cache.RealCookiesEarned >= 1000000000000000000000000000000000000) {
        CM.Sim.Win('You get nothing');
    }
    if (
        CM.Cache.RealCookiesEarned >= 1000000000000000000000000000000000000000
    ) {
        CM.Sim.Win('Humble rebeginnings');
    }
    if (
        CM.Cache.RealCookiesEarned >=
        1000000000000000000000000000000000000000000
    ) {
        CM.Sim.Win('The end of the world');
    }
    if (
        CM.Cache.RealCookiesEarned >=
        1000000000000000000000000000000000000000000000
    ) {
        CM.Sim.Win("Oh, you're back");
    }
    if (
        CM.Cache.RealCookiesEarned >=
    ...
    if (
        ...
        CM.Sim.Win('Smurf account');
    }
    if (
        CM.Cache.RealCookiesEarned >=
        1000000000000000000000000000000000000000000000000000000
    ) {
        CM.Sim.Win("If at first you don't succeed");
    }
...
CM.Sim.modifyBuildingPrice = (building, price) => {
    if (CM.Sim.Has('Season savings')) {
        price *= 0.99;
    }
    if (CM.Sim.Has("Santa's dominion")) {
        price *= 0.99;
    }
    if (CM.Sim.Has('Faberge egg')) {
        price *= 0.99;
    }
    if (CM.Sim.Has('Divine discount')) {
        price *= 0.99;
    }
    if (CM.Sim.Has('Fortune #100')) {
        price *= 0.99;
    }
    // if (CM.Sim.hasAura('Fierce Hoarder')) {
    //     price *= 0.98;
    // }
    ...
    if (Game.hasBuff('Crafty pixies')) {
        price *= 0.98;
    }
    if (Game.hasBuff('Nasty goblins')) {
        price *= 1.02;
    }
    if (building.fortune && CM.Sim.Has(building.fortune.name)) {
        price *= 0.93;
    }
...
CM.Sim.mouseCps = () => {
    let add = 0;
    if (CM.Sim.Has('Thousand fingers')) {
        add += 0.1;
    }
    if (CM.Sim.Has('Million fingers')) {
        add *= 5;
    }
    if (CM.Sim.Has('Billion fingers')) {
        add *= 10;
    }
    if (CM.Sim.Has('Trillion fingers')) {
        add *= 20;
    }
    if (CM.Sim.Has('Quadrillion fingers')) {
        add *= 20;
    }
    if (CM.Sim.Has('Quintillion fingers')) {
        add *= 20;
    }
    if (CM.Sim.Has('Sextillion fingers')) {
        add *= 20;
    }
    if (CM.Sim.Has('Septillion fingers')) {
        add *= 20;
    }
    if (CM.Sim.Has('Octillion fingers')) {
        add *= 20;
    }
    if (CM.Sim.Has('Nonillion fingers')) {
        add *= 20;
    }
    let num = 0;
    for (const i of Object.keys(CM.Sim.Objects)) {
        num += CM.Sim.Objects[i].amount;
    }
    num -= CM.Sim.Objects.Cursor.amount;
    add *= num;

    // Use CM.Sim.cookiesPs as function is always called after CM.Sim.CalculateGains()
    if (CM.Sim.Has('Plastic mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }
    if (CM.Sim.Has('Iron mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }
    if (CM.Sim.Has('Titanium mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }
    if (CM.Sim.Has('Adamantium mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }
    if (CM.Sim.Has('Unobtainium mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }
    if (CM.Sim.Has('Eludium mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }
    if (CM.Sim.Has('Wishalloy mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }
    if (CM.Sim.Has('Fantasteel mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }
    if (CM.Sim.Has('Nevercrack mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }
    if (CM.Sim.Has('Armythril mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }
    if (CM.Sim.Has('Technobsidian mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }
    if (CM.Sim.Has('Plasmarble mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }
    if (CM.Sim.Has('Miraculite mouse')) {
        add += CM.Sim.cookiesPs * 0.01;
    }

    if (CM.Sim.Has('Fortune #104')) {
        add += CM.Sim.cookiesPs * 0.01;
    }

    let mult = 1;
    if (CM.Sim.Has("Santa's helpers")) {
        mult *= 1.1;
    }
    if (CM.Sim.Has('Cookie egg')) {
        mult *= 1.1;
    }
    if (CM.Sim.Has('Halo gloves')) {
        mult *= 1.1;
    }
    if (CM.Sim.Has('Dragon claw')) {
        mult *= 1.03;
    }
        ...
        mult *=
            ...
            0.05 *
                Math.min(
                    ...
                    CM.Sim.Has('Luminous gloves') ? 20 : 10,
                );
    }
    ...
    // if (CM.Sim.auraMult('Dragon Cursor')) {
    //     mult *= 1.05;
    // }
    ...
    let out =
        ...
        Game.ComputeCps(
            1,
            CM.Sim.Has('Reinforced index finger') +
                CM.Sim.Has('Carpal tunnel prevention cream') +
                CM.Sim.Has('Ambidextrous'),
            add,
        );
...
CM.init = () => {
    ...
    if (Game.version !== CM.VersionMajor) {
        proceed = window.confirm(
            `Cookie Monster version ${CM.VersionMajor}.${CM.VersionMinor} is meant for Game version ${CM.VersionMajor}.  Loading a different version may cause errors.  Do you still want to load Cookie Monster?`,
        );
    }
    if (proceed) {
        CM.DelayInit();
...
CM.Footer.AddJscolor = () => {
    ...
    CM.Footer.Jscolor.setAttribute(
        'src',
        'https://aktanusa.github.io/CookieMonster/jscolor/jscolor.js',
    );
