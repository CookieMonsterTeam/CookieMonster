...
export default function CalculateAllPlotChances(minigame, auraMult) {
  ...
  for (let y = 0; y < 6; y++) for (let x = 0; x < 6; x++) {
    ...
    let ageMult; let weedMult; let range;
    const id = tile[0] - 1;
    switch (id) {
      case 7:
        ageMult = 1.03;
  ...
  for (let y = 0; y < 6; y++) for (let x = 0; x < 6; x++) {
    ...
    plotOutcomes[y][x] = CalculateSingleTileChances(minigame, tile[0] - 1, tile[1], neighResults, cardinals, boosts[y][x], dragonBoost);

    // save results
    CachePlotChances[y][x] = [];

    const emptyP = plotOutcomes[y][x].filter(o => o.plantId === -1).reduce((s, o) => s + o.p, 0);
    ...
    for (let id = 0; id < plantsById.length; id++) {
      ...
      if (matches.length > 1) {
        ...
        const maturityP = matches.filter(o => o.isMature === true).reduce((s, o) => s + o.p, 0) / plantP;
        CachePlotChances[y][x].push({ plantId: id, maturityP, p: plantP });
      }
      else if (matches.length === 1) CachePlotChances[y][x].push({ plantId: id, maturityP: matches[0].isMature ? 1 : 0, p: matches[0].p });
    }
